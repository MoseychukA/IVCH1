...
var pinId = ['D1','D2','D3','D4','D5', 'D12','D13','D14','D15','D16','D17','D18','D19','D21','D22','D23', 'D25','D26','D27', 'D32','D33','D34','D35','D36', 'D39'];
    ...
    if (this.readyState == 4) {
      if (this.status == 200) {
        if (this.responseXML != null) {
          ...
          try {
            var freeRamPer = Math.round(freeRam / 2857, 0); // MAX_FREE_MEM 285736 / 100 (ESP32)
            document.getElementById("dash-free-ram").innerHTML = freeRam + ' / ' + freeRamPer + '%';
          } catch (err) { }

          // SD Size
          try {
            var sdSize = this.responseXML.getElementsByTagName('sdSize')[0].childNodes[0].nodeValue;
          } catch (err) {
              sdSize = '-';
            }
          try {
            //var freeRamPer = Math.round(freeRam / 462, 0); // MAX_FREE_MEM 46224 / 100 (dev ESP8266)
            document.getElementById("sd-size").innerHTML = sdSize;
          } catch (err) { }
          ...
          if (currentPage == 'electro-page') {
            ...
            try {
              device('device-plita', 3, ui[2], 'Kitchen', 200, 500, 900, 1500);
            } catch (err) { }
            try {
              device('device-str',   3, ui[3], 'Washing', 60, 300, 1500, 2000);
            } catch (err) { }
            try {
              device('device-cond',  3, ui[4], 'Conditioner', 60, 100, 600, 1500);
            } catch (err) { }

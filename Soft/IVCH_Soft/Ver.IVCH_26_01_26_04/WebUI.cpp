...
// RTC (нужен для удаления логов старше N дней)
extern RealtimeClock rtc; // тип доступен через ваши заголовки (используется в проекте)
...
static bool applyGsmPresetByKey(const char* key)
{
	...
	for (size_t i = 0; i < sizeof(kGsmPresets) / sizeof(kGsmPresets[0]); i++) {
		if (strcmp(kGsmPresets[i].key, key) == 0) {
			...
			// 2) also set GSM operator index in syncData (so WEB shows correct operator)
...
// days-from-civil (UTC-like) for age compare (date only)
...
{
	if (!nm) return false;
	if (strlen(nm) != 12) return false; // 8.3:"IVyymmdd.LOG"
...
// ================== FIXED:findParam safe for multiple calls ==================
const char* WebUI::findParam(char* query, const char* key)
{
	// ring buffers so multiple findParam() calls in a row are safe
...
static void htmlHeader(EthernetClient& c, const __FlashStringHelper* title)
{
	...
	c.print(F("<title>WEB интерфейс ИВЧ - ")); c.print(title); c.print(F("</title>"));
...
static void pageLogList(EthernetClient& c)
{
	sendHtmlOkHeader(c);
	htmlHeader(c, F("ЛОГИ"));
...
{
	if (v < 1) return 1;
	if (v > 3650) return 3650; // up to 10 years
	return (uint16_t)v;
}
...
static uint32_t purgeOldLogs(uint16_t keepDays)
{
	...
	while (true) {
		...
		// close early (SD.remove works without open file)
		f.close();
		...
		// delete NO-RTC log if RTC is valid (optional)
		...
		int32_t fileDays = daysFromCivil(y, m, d);
		int32_t age = nowDays - fileDays;
...
{
	sendHtmlOkHeader(c);
	htmlHeader(c, F("УДАЛЕНИЕ СТАРЫХ ЛОГОВ"));
...
void WebUI::handleClient(EthernetClient& c)
{
	...
	// LOG clear (with confirmation) for selected file
	...
	// LOG purge old files (with confirmation)
	else if (isGet && strcmp(path, "/log/purge") == 0)
	{
		...
		// confirm -> purge
		SdLogClose(); // close current file to avoid deleting open handles (we do not delete today's file anyway)
		uint32_t delCount = purgeOldLogs(keepDays);

		// show result
		sendHtmlOkHeader(c);
		htmlHeader(c, F("УДАЛЕНИЕ ЛОГОВ"));
		c.print(F("<div class='box'><h3>Готово</h3><p>Удалено файлов:<code>"));
		c.print((unsigned long)delCount);
		c.print(F("</code></p><p><a href='/log'><button>К СПИСКУ ЛОГОВ</button></a></p></div>"));
		htmlFooter(c);
...
// -------------------- остальные методы (pageCfg/pageGsm/pageNotFound/apiStatus/apiConfig/actionCfgSave/actionGsmSave)
// -------------------- ОСТАВЛЕНЫ БЕЗ ИЗМЕНЕНИЙ относительно вашего файла --------------------
...
void WebUI::actionGsmSave(EthernetClient& c, char* params)
{
	...
	// 2) save gsm enable/operator/period
	bool en;
	uint8_t u8;

...
static const GsmPreset kGsmPresets[] = {
	// ВНИМАНИЕ:параметры могут отличаться по региону/тарифу.
...
static void htmlHeader(EthernetClient& c, const __FlashStringHelper* title)
{
	...
	// DENSE LAYOUT (плотнее по вертикали)
	c.print(F("<style>"
...
void WebUI::pageCfg(EthernetClient& c)
{
	...
	c.print(F("<p>PASS:<input type='password' name='apn_pass' value='")); htmlEscPrint(c, cfg.pass); c.print(F("'></p>"));
	...
	// INTERNET1 store (LanIfStore IF1)
	...
	// INTERNET2 store (LanIfStore IF2)
...
void WebUI::pageGsm(EthernetClient& c)
{
	...
	// Пресеты операторов (только МТС/МЕГАФОН/БИЛАЙН)
...
void WebUI::actionGsmSave(EthernetClient& c, char* params)
{
	...
	// 1) Применение пресета оператора
	...
	// 2) Обычное сохранение GSM enable/operator/period
	bool en;
	uint8_t u8;
	...
	// 3) Ручной ввод APN/USER/PASS/SERVER
	const char* apn = findParam(params, "apn");
	const char* apn_user = findParam(params, "apn_user");
	const char* apn_pass = findParam(params, "apn_pass");
	const char* ntp_server = findParam(params, "ntp_server");
...
//Важно: если у вас где - то ещё в проекте(например, в Menu.cpp) используется список операторов на 5 пунктов и делается gsmProviderIdx % 5,
//то там тоже нужно заменить на 3, иначе индексы будут “прыгать” и отображаться не тем оператором.

...
//    spi2.transfer16(READ_X /* X */);  // dummy X measure, 1st is always noisy
  ...
  digitalWrite(_pin_cs, HIGH);
  //spi2.endTransaction();
  //Serial4.printf("z=%d  ::  z1=%d,  z2=%d  ", z, z1, z2);
  if (z < 0) z = 0;
  if (z < Z_THRESHOLD) 
  { //  if ( !touched ) {
    // Serial4.println();
  ...
  //Serial4.printf("    z1=%d,z2=%d  ", z1, z2);
  //Serial4.printf("p=%d,  %d,%d  %d,%d  %d,%d", zraw,
    //data[0], data[1], data[2], data[3], data[4], data[5]);
  int16_t x = besttwoavg( data[0], data[2], data[4] );
  int16_t y = besttwoavg( data[1], data[3], data[5] );
  
  //Serial4.printf("    %d,%d", x, y);
  //Serial4.println();
  if (z >= Z_THRESHOLD) 
  {
  ...
  //  Serial4.print("xraw: "); Serial4.println(xraw);
 //   Serial4.print("yraw: "); Serial4.println(yraw);

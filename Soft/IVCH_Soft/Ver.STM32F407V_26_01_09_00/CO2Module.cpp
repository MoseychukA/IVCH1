...
void CO2Module::setupOutputs()
{
       ...
       if(bnd.LinkType == linkUnbinded) // не используется
       {
        ...
        if(bnd.LinkType == linkPortExpander)
        {
            ...
            if (bnd.RelayPin != UNBINDED_PIN)
            {
                WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.RelayPin, OUTPUT);
                WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.RelayPin, !bnd.Level);
            }
            if (bnd.AlertPin != UNBINDED_PIN)
            {
                WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.AlertPin, OUTPUT);
                WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.AlertPin, !bnd.Level);
            }
            if (bnd.VentPin != UNBINDED_PIN)
            {
                WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.VentPin, OUTPUT);
                WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.VentPin, !bnd.Level);
            }
            #endif
        }
    ...
    CO2Binding bnd = HardwareBinding->GetCO2Binding();
     ...
     if(bnd.LinkType == linkUnbinded) // не используется
     {
      ...
      if(bnd.LinkType == linkPortExpander)
      {
        ...
        if (pin != UNBINDED_PIN)
        {
            WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, pin, level);
        }
        #endif
      }

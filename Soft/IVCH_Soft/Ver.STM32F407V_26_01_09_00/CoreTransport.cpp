...
void CoreTransportClient::disconnect()
{
  ...
  if(!parent)
  {
	  ...
	  if (debug.WIFI_DEBUG_K == HIGH || debug.GSM_DEBUG_MODE_K == HIGH)
	  {
...
void CoreESPTransport::sendCommand(ESPCommands command)
{
  ...
  switch(command)
  {
    ...
    case cmdCIPSERVER:
    {
		...
		if (debug.WIFI_DEBUG_K == HIGH)
		{
			 Serial4.println(F("ESP: Starting TCP-server..."));
		}
      #endif
...
void CoreESPTransport::power(bool on)
{
  ...
  if(bnd.RebootPinLinkType != linkUnbinded && bnd.RebootPin != UNBINDED_PIN)
  {
      ...
      if(bnd.RebootPinLinkType == linkPortExpander)
      {
          ...
          WORK_STATUS.MCP_SPI_PinWrite(bnd.RebootPinMCPAddress,bnd.RebootPin,on ? bnd.PowerOnLevel: !bnd.PowerOnLevel);
            ...
            WORK_STATUS.MCP_I2C_PinWrite(bnd.RebootPinMCPAddress,bnd.RebootPin, on ? bnd.PowerOnLevel: !bnd.PowerOnLevel);
              ...
              WORK_STATUS.PCF8575_PinMode(bnd.RebootPinMCPAddress, bnd.RebootPin, OUTPUT);
              WORK_STATUS.PCF8575_PinWrite(bnd.RebootPinMCPAddress, bnd.RebootPin, on ? bnd.PowerOnLevel : !bnd.PowerOnLevel);
         #endif
      }
  } // if(bnd.RebootPinLinkType != linkUnbinded && bnd.RebootPin != UNBINDED_PIN)    
}
...
void CoreSIM800Transport::sendCommand(const String& command, bool addNewLine)
{
 ...
 SysDebug debug = HardwareBinding->GetSysDebug();
  ...
  readFromStream();
   ...
   if (debug.GSM_DEBUG_MODE_K == HIGH)
   {
	   DEBUG_LOG(F("SIM800: ==> "));
	   DEBUG_LOGLN(command);
   }
...
void CoreSIM800Transport::processIncomingCall(const String& line)
{
 // приходит строка вида
  ...
  if (MemRead(SMS_NUMBER_EEPROM_ADDR1) == 43 && isKnownNumber == false)
  {
      knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(1);
      if (knownNumber.startsWith(ring))
      {
          isKnownNumber = true;
          MainController->GetSettings()->SetNumberIncomingSMS(knownNumber);
      }
  }
  if (MemRead(SMS_NUMBER_EEPROM_ADDR2) == 43 && isKnownNumber == false)
  {
      knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(2);
      if (knownNumber.startsWith(ring))
      {
          isKnownNumber = true;
          MainController->GetSettings()->SetNumberIncomingSMS(knownNumber);
  ...
  if (MemRead(SMS_NUMBER_EEPROM_ADDR3) == 43 && isKnownNumber == false)
  {
      knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(3);
      if (knownNumber.startsWith(ring))
      {
          isKnownNumber = true;
          MainController->GetSettings()->SetNumberIncomingSMS(knownNumber);
  ...
  if (MemRead(SMS_NUMBER_EEPROM_ADDR4) == 43 && isKnownNumber == false)
  {
      knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(4);
      if (knownNumber.startsWith(ring))
      {
          isKnownNumber = true;
          MainController->GetSettings()->SetNumberIncomingSMS(knownNumber);
...
void CoreSIM800Transport::processCMT(const String& pdu)
{
  if(pdu.length())
  {
    ...
    if (sms.IsDecodingSucceed)
    {
        ...
        String knownNumber = "";
        if (MemRead(SMS_NUMBER_EEPROM_ADDR1) == 43 && anyKnownNumbersFound == false)
        {
            knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(1);
        ...
        if (MemRead(SMS_NUMBER_EEPROM_ADDR2) == 43 && anyKnownNumbersFound == false)
        {
            knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(2);
            if (knownNumber.startsWith(sms.SenderNumber))
            {
        ...
        if (MemRead(SMS_NUMBER_EEPROM_ADDR3) == 43 && anyKnownNumbersFound == false)
        {
            knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(3);
            if (knownNumber.startsWith(sms.SenderNumber))
            {
        ...
        if (MemRead(SMS_NUMBER_EEPROM_ADDR4) == 43 && anyKnownNumbersFound == false)
        {
            knownNumber = MainController->GetSettings()->GetSmsPhoneNumber(4);
            if (knownNumber.startsWith(sms.SenderNumber))
            {
...
void CoreSIM800Transport::dumpReceiveBuffer()
{
	...
	if (debug.GSM_DEBUG_MODE_K == HIGH)
	{
		 Serial4.print(F("SIM800 RECEIVE BUFFER, SIZE="));
		 Serial4.println(receiveBuffer.size());

		for (size_t i = 0; i < receiveBuffer.size(); i++)
		{
			 Serial4.print(WorkStatus::ToHex(receiveBuffer[i]));
			 Serial4.print(" ");
		}
...
void CoreSIM800Transport::power(bool on)
{
  ...
  if(gbnd.RebootPinLinkType != linkUnbinded && gbnd.RebootPin != UNBINDED_PIN)
  {
      ...
      if(gbnd.RebootPinLinkType == linkPortExpander)
      {
        #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
         ...
         #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
            WORK_STATUS.PCF8575_PinMode(gbnd.RebootPinMCPAddress, gbnd.RebootPin, OUTPUT);
            WORK_STATUS.PCF8575_PinWrite(gbnd.RebootPinMCPAddress, gbnd.RebootPin, on ? gbnd.PowerOnLevel : !gbnd.PowerOnLevel);
         #endif
      }
        ...
        {          
          ...
          {            
            ...
            if(gbnd.PowerkeyLinkType != linkUnbinded && gbnd.PowerkeyPin != UNBINDED_PIN)
            {
                  ...
                  if(gbnd.PowerkeyLinkType == linkPortExpander)
                  {
                    #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
                    ...
                    #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
                      WORK_STATUS.PCF8575_PinWrite(gbnd.RebootPinMCPAddress, gbnd.PowerkeyPin, !gbnd.PowerkeyOnLevel);
                    #endif
                  }
            } // if(gbnd.PowerkeyLinkType != linkUnbinded && gbnd.PowerkeyPin != UNBINDED_PIN)
...
void CoreSIM800Transport::powerkeyInit()
{
  ...
  if(gbnd.PowerkeyLinkType != linkUnbinded && gbnd.PowerkeyPin != UNBINDED_PIN)
  {
    ...
    #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
            WORK_STATUS.PCF8575_PinMode(gbnd.PowerkeyMCPAddress, gbnd.PowerkeyPin, OUTPUT);
    #endif
    }
...
bool CoreSIM800Transport::sendSMS(const String& phoneNumber, const String& message, bool isFlash)
{
  ...
  SysDebug debug = HardwareBinding->GetSysDebug();
	...
	{
		 Serial4.print("+++ ");
		 Serial4.print(String(queuedSMS.isFlash));
		 Serial4.print(" ");
		 Serial4.print(phoneNumber.c_str());
		 Serial4.print(" ");
		 Serial4.println(message.c_str());
	}
#endif

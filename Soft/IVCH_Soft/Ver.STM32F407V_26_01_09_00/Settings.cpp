...
void GlobalSettings::setup()
{
  ...
  0
    ...
    //==
    uint32_t sAddrN = WATERING_COUNT_ADDRESS;
    wateringCount = read8(sAddrN, 0); sAddrN++;

    uint32_t sAddrI = WATERING_INTERVAL_ADDRESS;
    wateringInterval = read8(sAddrI, 0); sAddrI++;
    ...
    for(uint8_t idx=0;idx < WATER_RELAYS_COUNT; idx++)
    {
      ...
      if (wateringChannels[idx].countBorder == 0xFF)
          wateringChannels[idx].countBorder = 0;

      if (wateringChannels[idx].intervalBorder == 0xFF)
          wateringChannels[idx].intervalBorder = 0;

      if (wateringChannels[idx].intervalSecond == 0xFF)
          wateringChannels[idx].intervalSecond = 0;
...
//--------------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------
HumidityDrainingSettings GlobalSettings::GetHumidityDrainingSettings(uint8_t channel)
{
	HumidityDrainingSettings result;
	memset(&result, 0, sizeof(HumidityDrainingSettings));

	result.active = true;
	result.histeresis = 5;

	uint32_t addr = WM_HUMIDITY_DRAINING_SETTINGS_ADDRESS + channel * (sizeof(HumidityDrainingSettings) + sizeof(uint16_t));
	if (!checkHeader(addr))
		return result;
	...
	uint8_t* ptr = (uint8_t*)&result;
	for (size_t i = 0; i < sizeof(HumidityDrainingSettings); i++)
	{
		*ptr++ = MemRead(addr++);
	}
...
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetHumidityDrainingSettings(uint8_t channel, HumidityDrainingSettings& val)
{
	uint32_t addr = WM_HUMIDITY_DRAINING_SETTINGS_ADDRESS + channel * (sizeof(HumidityDrainingSettings) + sizeof(uint16_t));
	writeHeader(addr);
	addr += sizeof(int16_t);
...
HeatSettings GlobalSettings::GetHeatSettings(uint8_t channel)
{
  ...
  result.active_night = false;
  result.startTime = HEAT_DEFAULT_START_TIME;
  result.duration = HEAT_DEFAULT_DURATION;
  result.nightTemp = HEAT_DEFAULT_NIGHT_TEMP;
...
//--------------------------------------------------------------------------------------------------------------------------------------
uint16_t GlobalSettings::GetChannelWateringCount(uint8_t idx)
{
    return wateringChannels[idx].countBorder;
}

//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetChannelWateringCount(uint8_t idx, uint8_t val)
{
    wateringChannels[idx].countBorder = val;
    uint32_t writeAddr = WATERING_CHANNELS_SETTINGS_EEPROM_ADDR + idx * sizeof(WateringChannelOptions) + 7; // с восьмого байта в структуре идёт значение
    MemWrite(writeAddr, val);
}
//--------------------------------------------------------------------------------------------------------------------------------------

uint8_t GlobalSettings::GetChannelWateringInterval(uint8_t idx)
{
    return wateringChannels[idx].intervalBorder;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetChannelWateringInterval(uint8_t idx, uint8_t val)
{
    wateringChannels[idx].intervalBorder = val;
    uint32_t writeAddr = WATERING_CHANNELS_SETTINGS_EEPROM_ADDR + idx * sizeof(WateringChannelOptions) + 8; // с девятого байта в структуре идёт значение
    MemWrite(writeAddr, val);
}

//--------------------------------------------------------------------------------------------------------------------------------------
uint8_t  GlobalSettings::GetChannelWateringSecond(uint8_t idx)
{
    return wateringChannels[idx].intervalSecond;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetChannelWateringSecond(uint8_t idx, uint8_t val)
{
    wateringChannels[idx].intervalSecond = val;

    uint32_t writeAddr =
        WATERING_CHANNELS_SETTINGS_EEPROM_ADDR
        + (uint32_t)idx * sizeof(WateringChannelOptions)
        + (uint32_t)offsetof(WateringChannelOptions, intervalSecond);
...
//--------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
int GlobalSettings::GetButtonPhoneNumber()
{
    return ButtonPhoneNumber;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetButtonPhoneNumber(int v)
{
    ButtonPhoneNumber = v;

}
//--------------------------------------------------------------------------------------------------------------------------------------
String GlobalSettings::GetNumberIncomingSMS()
{
    return NumberIncomingSMS;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetNumberIncomingSMS(const String& v)
{
    NumberIncomingSMS = v;
    writeString(SMS_NUMBER_EEPROM_ADDR, v, 15);
}
//--------------------------------------------------------------------------------------------------------------------------------------
String GlobalSettings::GetSmsPhoneNumber(uint8_t val)
{
    switch (val)
    {
    case 1:
        smsPhoneNumber = readString(SMS_NUMBER_EEPROM_ADDR1, 13);
        break;
    case 2:
        smsPhoneNumber = readString(SMS_NUMBER_EEPROM_ADDR2, 13);
        break;
    case 3:
        smsPhoneNumber = readString(SMS_NUMBER_EEPROM_ADDR3, 13);
        break;
    case 4:
        smsPhoneNumber = readString(SMS_NUMBER_EEPROM_ADDR4, 13);
        break;
    }
    return smsPhoneNumber;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetSmsPhoneNumber(const String& v, uint8_t val)
{
    smsPhoneNumber = v;
    switch (val)
    {
    case 1:
        writeString(SMS_NUMBER_EEPROM_ADDR1, v, 15);
        break;
    case 2:
        writeString(SMS_NUMBER_EEPROM_ADDR2, v, 15);
        break;
    case 3:
        writeString(SMS_NUMBER_EEPROM_ADDR3, v, 15);
        break;
    case 4:
        writeString(SMS_NUMBER_EEPROM_ADDR4, v, 15);
        break;
    }
}
...
//--------------------------------------------------------------------------------------------------------------------------------------
uint8_t GlobalSettings::GetWateringCount()
{
    return wateringCount;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetWateringCount(uint8_t val)
{
    wateringCount = val;
    MemWrite(WATERING_COUNT_ADDRESS, val);
}

//--------------------------------------------------------------------------------------------------------------------------------------
uint8_t GlobalSettings::GetWateringInterval()
{
    return wateringInterval;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void GlobalSettings::SetWateringInterval(uint8_t val)
{
    wateringInterval = val;
    MemWrite(WATERING_INTERVAL_ADDRESS, val);
}

...
void PhModule::Setup()
{
  ...
  if(bnd.LinkTypeOutputs == linkPortExpander)
  {
        ...
        if(bnd.FlowAddPin != UNBINDED_PIN)
        {
          WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressOutputs,bnd.FlowAddPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddressOutputs,bnd.FlowAddPin,!bnd.OutputsLevel);
        }

        if(bnd.PhPlusPin != UNBINDED_PIN)
        {
          WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressOutputs,bnd.PhPlusPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddressOutputs,bnd.PhPlusPin,!bnd.OutputsLevel);
        }

        if(bnd.PhMinusPin != UNBINDED_PIN)
        {
          WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressOutputs,bnd.PhMinusPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddressOutputs,bnd.PhMinusPin,!bnd.OutputsLevel);
        }

        if(bnd.PhMixPin != UNBINDED_PIN)
        {
          WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressOutputs,bnd.PhMixPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddressOutputs,bnd.PhMixPin,!bnd.OutputsLevel);
        }
  ...
  if(bnd.LinkTypeInputs == linkPortExpander)
  {
    #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
      ...
      if(bnd.FlowLevelSensorPin != UNBINDED_PIN)
      {
        WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressInputs,bnd.FlowLevelSensorPin,INPUT);
      }

      if(bnd.FlowLevelSensorPin2 != UNBINDED_PIN)
      {
        WORK_STATUS.PCF8575_PinMode(bnd.MCPAddressInputs,bnd.FlowLevelSensorPin2,INPUT);
      }
  ...
  PHBinding bnd = HardwareBinding->GetPHBinding();
     ...
     if(bnd.LinkTypeInputs == linkUnbinded) // не используется
      ...
      if(bnd.LinkTypeInputs == linkPortExpander)
      {
            ...
            if(pin != UNBINDED_PIN)
            {
              return WORK_STATUS.PCF8575_PinRead(bnd.MCPAddressInputs,pin);
            }
          #endif
      }  
    ...
    PHBinding bnd = HardwareBinding->GetPHBinding();
     ...
     if(bnd.LinkTypeOutputs == linkUnbinded) // не используется
      ...
      if(bnd.LinkTypeOutputs == linkPortExpander)
      {
            ...
            if(pin != UNBINDED_PIN)
            {
              WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddressOutputs,pin,level);
            }
          #endif
      }
}

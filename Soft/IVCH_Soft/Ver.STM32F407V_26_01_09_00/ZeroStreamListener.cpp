    ...
    else
    {
      ...
      else
      {
        ...
        if(t == SMS_NUMBER_COMMAND) // номер телефона для управления по СМС
        {
          ...
          PublishSingleton << PARAM_DELIMITER << MainController->GetSettings()->GetNumberIncomingSMS();
        }
    ...
    else
    {
      if(argsCnt < 2)
      {
        ...
        if(t == RESET_COMMAND)
        {
            ...
            // Записываем магическое значение ключа в регистр NVIC_APINT для осуществления программного сброса
            __set_CONTROL(__get_CONTROL() | 0x0);     // Очистка CONTROL регистра (необязательно, но рекомендуется)
            __DSB();                                  // Барьер синхронизации памяти
            __ISB();                                  // Инструкция барьерного состояния
      ...
      if(t == ADD_COMMAND) // запросили регистрацию нового модуля
       ...
       if(t == SMS_NUMBER_COMMAND) // номер телефона для управления по SMS
       {
          ...
          sett->SetNumberIncomingSMS(command.GetArg(1));
          PublishSingleton.Flags.Status = true;
          PublishSingleton = SMS_NUMBER_COMMAND; 

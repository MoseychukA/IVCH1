...
void Thermostat::setup(uint8_t _channel)
{
	...
	if (debug.THERMOSTAT_DEBUG_K == HIGH)
	{
		Serial4.print(F("THERMOSTAT SETUP: CHANNEL #"));
		Serial4.println(_channel);
	}
  #endif
...
// настраиваем выхода
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.Pin != UNBINDED_PIN)
    {
          ...
          if (bnd.Pin != UNBINDED_PIN)
          {
              WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.Pin, OUTPUT);
              WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin, !bnd.Level);
          }
    #endif
  }
...
void Thermostat::setState(bool on)
{
  ...
  if (debug.THERMOSTAT_DEBUG_K == HIGH)
  {
	  Serial4.print(F("THERMOSTAT SET STATE, CHANNEL #"));
	  Serial4.print(channel);
	  Serial4.print(F("; STATE = "));
	  Serial4.println(on);
  }
  #endif
...
if(bnd.LinkType == linkDirect)
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.Pin != UNBINDED_PIN)
    {
        ...
        if (bnd.Pin != UNBINDED_PIN)
        {
            WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin, level);
        }
    #endif
  }
}
...
void Thermostat::reloadSettings()
{
	...
	if (debug.THERMOSTAT_DEBUG_K == HIGH)
	{
		Serial4.print(F("THERMOSTAT RELOAD SETTINGS: CHANNEL #"));
		Serial4.println(channel);
	}
  #endif
...
void Thermostat::check()
{
	...
	if (debug.THERMOSTAT_DEBUG_K == HIGH)
	{
		Serial4.print(F("THERMOSTAT CHECK WANT ON: CHANNEL #"));
		Serial4.println(channel);
	}
  #endif
  ...
  if(temp.Value == NO_TEMPERATURE_DATA)
  {
	  ...
	  if (debug.THERMOSTAT_DEBUG_K == HIGH)
	  {
		  Serial4.print(F("THERMOSTAT: NO SENSOR TEMPERATURE, CHANNEL #"));
		  Serial4.println(channel);
	  }
      #endif
  ...
  if(tVal >= (tOnBorder + 10l*settings.histeresis))
  {
    // мы превысили порог выключения, надо выключать !!!
	  ...
	  if (debug.THERMOSTAT_DEBUG_K == HIGH)
	  {
		  Serial4.print(F("THERMOSTAT: CHANNEL #"));
		  Serial4.print(channel);
		  Serial4.println(F(" WANTS OFF!"));
	  }
    #endif
...
void Vent::setup(uint8_t _channel)
{
	SysDebug debug = HardwareBinding->GetSysDebug();
  #ifdef VENT_DEBUG
	if (debug.VENT_DEBUG_K == HIGH)
	{
		Serial4.print(F("VENT SETUP: CHANNEL #"));
		Serial4.println(_channel);
	}
  #endif
...
void Vent::setState(bool on)
{
  ...
  if (debug.VENT_DEBUG_K == HIGH)
  {
	  Serial4.print(F("VENT SET STATE, CHANNEL #"));
	  Serial4.print(channel);
	  Serial4.print(F("; STATE = "));
	  Serial4.println(on);
  }
...
void Vent::reloadSettings()
{
	...
	if (debug.VENT_DEBUG_K == HIGH)
	{
		Serial4.print(F("VENT RELOAD SETTINGS: CHANNEL #"));
		Serial4.println(channel);
	}
  #endif
    ...
    case ventCheckOff:
    {
      ...
      if(millis() - workStartedAt > maxWorkTime)
      {
         // достигли максимального времени работы, надо отдохнуть !!!
		  ...
		  if (debug.VENT_DEBUG_K == HIGH)
		  {
			  Serial4.print(F("VENT WANTS REST: CHANNEL #"));
			  Serial4.println(channel);
		  }
        #endif
      ...
      if(millis() - tempTimer > VENT_UPDATE_INTERVAL)
      {
        ...
        if(temp.Value == NO_TEMPERATURE_DATA)
        {
          // нет температуры, переключаемся на режим ожидания, и выключаем всё
			...
			if (debug.VENT_DEBUG_K == HIGH)
			{
				Serial4.print(F("VENT: NO SENSOR TEMPERATURE, CHANNEL #"));
				Serial4.println(channel);
			}
          #endif
        ...
        else
        {
          ...
          if(tVal <= tOffBorder)
          {
            // достигли нижнего порога, надо выключать!
			  ...
			  if (debug.VENT_DEBUG_K == HIGH)
			  {
				  Serial4.print(F("VENT WANT OFF: CHANNEL #"));
				  Serial4.println(channel);
			  }
            #endif
            ...
            if(millis() - workStartedAt > minWorkTime)
            {
              // проработали больше, чем минимальное время
				...
				if (debug.VENT_DEBUG_K == HIGH)
				{
					Serial4.print(F("VENT OFF: CHANNEL #"));
					Serial4.println(channel);
				}
              #endif
            ...
            else
            {
			  ...
			  if (debug.VENT_DEBUG_K == HIGH)
			  {
				  Serial4.print(F("VENT NEED ADDITIONAL TIME: CHANNEL #"));
				  Serial4.print(channel);
				  Serial4.print(F(", TIME = "));
				  Serial4.println(addInterval);
			  }
              #endif            
    ...
    case ventWaitAddInterval:
    {
      ...
      if(millis() - workStartedAt > addInterval)
      {
        // интервал прошёл
		  ...
		  if (debug.VENT_DEBUG_K == HIGH)
		  {
			  Serial4.print(F("VENT ADDITIONAL TIME DONE: CHANNEL #"));
			  Serial4.println(channel);
		  }
        #endif
    ...
    case ventRest:
    {
      ...
      if(millis() - workStartedAt > restTime)
      {
         // отдохнули, можно начинать опрашивать датчик температуры
		  ...
		  if (debug.VENT_DEBUG_K == HIGH)
		  {
			  Serial4.print(F("VENT REST DONE: CHANNEL #"));
			  Serial4.println(channel);
		  }
        #endif
...
void Vent::checkWantOn()
{
	...
	if (debug.VENT_DEBUG_K == HIGH)
	{
		Serial4.print(F("VENT CHECK WANT ON: CHANNEL #"));
		Serial4.println(channel);
	}
  #endif
...
void HumiditySpray::setup(uint8_t _channel)
{
	...
	if (debug.HUMIDITY_SPRAY_DEBUG_K == HIGH)
	{
		Serial4.print(F("SPRAY SETUP: CHANNEL #"));
		Serial4.println(_channel);
	}
  #endif
...
void HumiditySpray::setState(bool on)
{
  ...
  if (debug.HUMIDITY_SPRAY_DEBUG_K == HIGH)
  {
	  Serial4.print(F("SPRAY SET STATE, CHANNEL #"));
	  Serial4.print(channel);
	  Serial4.print(F("; STATE = "));
	  Serial4.println(on);
  }
...
void HumiditySpray::reloadSettings()
{
	...
	if (debug.HUMIDITY_SPRAY_DEBUG_K == HIGH)
	{
		Serial4.print(F("SPRAY RELOAD SETTINGS: CHANNEL #"));
		Serial4.println(channel);
	}
  #endif
    ...
    case hsmIdle: // в режиме ожидания
    {
      if(canWork()) // можем работать в этом диапазоне
      {
          ...
          if(hum.Value == NO_TEMPERATURE_DATA)
          {
			  ...
			  if (debug.HUMIDITY_SPRAY_DEBUG_K == HIGH)
			  {
				  Serial4.print(F("SPRAY: NO SENSOR DATA, CHANNEL #"));
				  Serial4.println(channel);
			  }
              #endif
        ...
        else
        {
          ...
          if(tVal >= tOffBorder)
          {
            // достигли нижнего порога, надо выключать!
			  ...
			  if (debug.HUMIDITY_SPRAY_DEBUG_K == HIGH)
			  {
				  Serial4.print(F("SPRAY WANT OFF: CHANNEL #"));
				  Serial4.println(channel);
			  }
            #endif
...
//--------------------------------------------------------------------------------------------------------------------------------------
HumidityDraining::HumidityDraining()
{
	channel = 0;
	settings.active = false;
	onFlag = false;
	machineState = hdrIdle;
	workMode = hdrAuto;
}
//--------------------------------------------------------------------------------------------------------------------------------------
void HumidityDraining::setup(uint8_t _channel)
{
	SysDebug debug = HardwareBinding->GetSysDebug();
#ifdef DRAINING_DEBUG
	if (debug.DRAINING_DEBUG_K == HIGH)
	{
		Serial4.print(F("DRAINING SETUP: CHANNEL #"));
		Serial4.println(_channel);
	}
#endif
	...
	// настраиваем выхода
	HumidityDrainingBinding bnd = HardwareBinding->GetHumidityDrainingBinding(channel);
...
//--------------------------------------------------------------------------------------------------------------------------------------
void HumidityDraining::setState(bool on)
{
	if (on && onFlag) // попросили включиться, но мы уже включены
		return;
	...
	if (debug.DRAINING_DEBUG_K == HIGH)
	{
		Serial4.print(F("DRAINING SET STATE, CHANNEL #"));
		Serial4.print(channel);
		Serial4.print(F("; STATE = "));
		Serial4.println(on);
	}
...
//--------------------------------------------------------------------------------------------------------------------------------------
void HumidityDraining::reloadSettings()
{
	SysDebug debug = HardwareBinding->GetSysDebug();
#ifdef DRAINING_DEBUG
	if (debug.DRAINING_DEBUG_K == HIGH)
	{
		Serial4.print(F("DRAINING RELOAD SETTINGS: CHANNEL #"));
		Serial4.println(channel);
	}
#endif

	GlobalSettings* sett = MainController->GetSettings();
	settings = sett->GetHumidityDrainingSettings(channel);
...
//--------------------------------------------------------------------------------------------------------------------------------------
void HumidityDraining::update()
{
	if (workMode == hdrManual) // ручной режим работы, ничего не надо делать
	{
		return;
	}
	...
	switch (machineState)
	{
	case hdrIdle: // в режиме ожидания
	{
		if (canWork()) // можем работать в этом диапазоне
		{
			// проверяем влажность
			...
			if (hum.Value == NO_TEMPERATURE_DATA)
			{
				...
				if (debug.DRAINING_DEBUG_K == HIGH)
				{
					Serial4.print(F("DRAINING: NO SENSOR DATA, CHANNEL #"));
					Serial4.println(channel);
				}
             #endif
			...
			int32_t tOnBorder = 100l * settings.drainingOnValue;
			tOnBorder -= 10l * settings.histeresis;

			if (tVal >= tOnBorder)
			{
				// влажность выше уставки, надо включать !!!
               #ifdef DRAINING_DEBUG
				if (debug.DRAINING_DEBUG_K == HIGH)
				{
					Serial4.print(F("DRAINING: CHANNEL #"));
					Serial4.print(channel);
					Serial4.println(F(" WANTS ON!"));
				}
              #endif
	...
	case hdrCheckOff:
	{
		...
		else
		{
			...
			int32_t tOffBorder = 100l * settings.drainingOffValue;
			tOffBorder += 10l * settings.histeresis;
			...
			if (tVal <= tOffBorder) 
			{
				...
				if (debug.DRAINING_DEBUG_K == HIGH)
				{
					Serial4.print(F("DRAINING WANT OFF: CHANNEL #"));
					Serial4.println(channel);
				}
              #endif
...
// настраиваем выхода
  ...
  if (debug.CYCLE_VENT_DEBUG_K == HIGH)
  {
	  Serial4.print("CYCLE VENT CHANNEL #");
	  Serial4.print(channel);
	  Serial4.println(" CycleVent::setup()");
  }
  #endif
}
//--------------------------------------------------------------------------------------------------------------------------------------
...
void CycleVent::disable()
{
  ...
  if (debug.CYCLE_VENT_DEBUG_K == HIGH)
  {
	  Serial4.print("CYCLE VENT CHANNEL #");
	  Serial4.print(channel);
	  Serial4.println(" CycleVent::disable()");
  }
  #endif
...
void CycleVent::enable()
{
  ...
  if (debug.CYCLE_VENT_DEBUG_K == HIGH)
  {
	  Serial4.print("CYCLE VENT CHANNEL #");
	  Serial4.print(channel);
	  Serial4.println(" CycleVent::enable()");
  }
  #endif
      ...
      if(canWork(sett))
      {
        // надо включать!
		  ...
		  if (debug.CYCLE_VENT_DEBUG_K == HIGH)
		  {
			  Serial4.print("CYCLE VENT CHANNEL #");
			  Serial4.print(channel);
			  Serial4.println(" WANTS ON FROM  cvmIdle!");
		  }
        #endif
...
void RainSensor::setup()
{
  ...
  if(bnd.WorkMode == wrsDirect) // датчик, подключенный напрямую к контроллеру
  {
      ...
      if(bnd.LinkType != linkUnbinded && bnd.Pin != UNBINDED_PIN) // есть привязка управления
      {
          ...
          if(bnd.LinkType == linkPortExpander)
          {
              #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
              ...
              #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
                WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.Pin, INPUT);
                #endif
          }
...
void RainSensor::update()
{
  ...
  {
    if((millis() - lastDataAt) > 2ul*WIND_RAIN_UPDATE_INTERVAL)     //WIND_RAIN_UPDATE_INTERVAL
    {
  ...
  if(bnd.WorkMode == wrsDirect)       // датчик подключен напрямую к контроллеру
  {
    ...
    // он выставляет определённый уровень на пине. При этом, если уровень не менялся очень долго - сбрасываем флаг.
     ...
     // обновляем достаточно часто
 /*    static uint32_t upd = 0;*/
     if(millis() - upd >= WIND_RAIN_CONTROL_INTERVAL)
     {
         ...
         if(bnd.LinkType != linkUnbinded && bnd.Pin != UNBINDED_PIN)
         {
            ...
            if(bnd.LinkType == linkDirect)
              ...
              if(bnd.LinkType == linkPortExpander)
              {
                ...
                #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
                    thisRainFlag = WORK_STATUS.PCF8575_PinRead(bnd.MCPAddress, bnd.Pin) == bnd.Level;
                #endif
              }
              ...
              {
                lastDataAt = millis();
                rainFlag = true;        // запоминаем флаг дождя
              }
...
#ifdef USE_HUMIDITY_DRAINING_MODULE
//--------------------------------------------------------------------------------------------------------------------------------------
void LogicManageModuleClass::ReloadHumidityDrainingSettings()
{
	draining1.reloadSettings();
	draining2.reloadSettings();
	draining3.reloadSettings();
}
//--------------------------------------------------------------------------------------------------------------------------------------
void LogicManageModuleClass::makeDrainingDecision()
{
	draining1.update();
	draining2.update();
	draining3.update();
...
void Shadow::on()
{
	...
	if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
	{
		Serial4.print("SHADOW wants ON, TIMER interval: drive #");
		Serial4.print(channel);
		Serial4.print(", interval=");
		Serial4.println(timerInterval);
	}
...
void Shadow::turnValve(ValveTurnMode mode)
{
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.Pin2 != UNBINDED_PIN)
    {
          ...
          if (bnd.Pin1 != UNBINDED_PIN)
          {
              WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin1, leftChannelLevel);
          }
          if (bnd.Pin2 != UNBINDED_PIN)
          {
              WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin2, rightChannelLevel);
          }
    #endif
  }
...
void Shadow::off()
{
	...
	if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
	{
		...
		Serial4.print("Shadow wants OFF, TIMER interval: drive #");
		Serial4.print(channel);
		Serial4.print(", interval=");
		Serial4.println(timerInterval);
	}
...
void Shadow::setup(uint8_t _channel, int16_t _driveWorkTime)
{
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if (bnd.Pin1 != UNBINDED_PIN)
    {
        WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.Pin1, OUTPUT);
        WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin1, !bnd.Level);
    }
    if (bnd.Pin2 != UNBINDED_PIN)
    {
        WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.Pin2, OUTPUT);
        WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin2, !bnd.Level);
    }
    #endif
  }
...
void LogicManageModuleClass::makeShadowDecision(Shadow& shadow, ShadowSettings& section, ShadowDriveCommand& decision)
{
	...
	if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
	{
		...
		Serial4.print("Section lux: ");
		Serial4.println(sectionLux);
	}
	#endif // SHADOW_MODULE_DEBUG
	...
	if(sectionLux == NO_LUMINOSITY_DATA)
	{
		...
		if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
		{
			...
			Serial4.println("NO SECTION LUX!!!");
		}
		#endif // SHADOW_MODULE_DEBUG
	...
	if(sectionLux >= section.lux)
	{
		...
		if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
		{
			...
			Serial4.println("WANT SHADOW ON!!!");
		}
		#endif // SHADOW_MODULE_DEBUG
	...
	if(sectionLux <= (section.lux - histeresis))
	{
		...
		if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
		{
			...
			Serial4.println("WANT SHADOW OFF!!!");
		}
		#endif // SHADOW_MODULE_DEBUG
...
{	
	...
	if (debug.SHADOW_MODULE_DEBUG_K == HIGH)
	{
		...
		Serial4.println("MAKE SHADOW DECISION!");
	}
	#endif // SHADOW_MODULE_DEBUG
...
void Heater::turnPump(bool _on)
{
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.PumpPin != UNBINDED_PIN)
    {
          ...
          if (bnd.PumpPin != UNBINDED_PIN)
          {
              WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.PumpPin, level);
          }
    #endif
  }
}
  ...
  if(onMyWay)
  {
      ...
      if(!timerCounter)
      {
        // закончили движение за установленное время
		  ...
		  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
		  {
			  Serial4.print("Heater #");
			  Serial4.print(channel);
			  Serial4.print(" VALVE DONE, position is: ");
			  Serial4.println(currentPosition);
		  }
        ...
        if(!currentPosition)
        {
			...
			if (debug.HEAT_MODULE_DEBUG_K == HIGH)
			{
				Serial4.println("Heater::update(), valve closed, turn pump OFF.");
			}
          #endif // HEAT_MODULE_DEBUG   
        ...
        else
        {
			...
			if (debug.HEAT_MODULE_DEBUG_K == HIGH)
			{
				Serial4.println("Heater::update(), valve opened, turn pump ON.");
			}
          #endif // HEAT_MODULE_DEBUG
 ...
 if(bnd.LinkType == linkDirect)
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.PumpPin != UNBINDED_PIN)
    {
         ...
         if (bnd.PumpPin != UNBINDED_PIN)
         {
             WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.PumpPin, OUTPUT);
             WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.PumpPin, !bnd.Level);
         }
...
void Heater::open(uint8_t percents)
{
  ...
  #ifdef HEAT_MODULE_DEBUG
    Serial4.print(channel);
    Serial4.print(" Heater::open(), requested position: %");
    Serial4.println(percents);
  #endif // HEAT_MODULE_DEBUG
  */
...
/*
  #ifdef HEAT_MODULE_DEBUG
    Serial4.print(channel);
    Serial4.println(" Heater::open(), SAME POSITION!");
  #endif // HEAT_MODULE_DEBUG
*/
    ...
    if(!currentPosition)
    {
      ...
      #ifdef HEAT_MODULE_DEBUG
        Serial4.print(channel);
        Serial4.println(" Heater::open(), same position, valve closed, turn pump OFF.");
      #endif // HEAT_MODULE_DEBUG   
      */
    ...
    else
    {
      ...
      #ifdef HEAT_MODULE_DEBUG
        Serial4.print(channel);
        Serial4.println(" Heater::open(), same position, valve opened, turn pump ON.");
      #endif // HEAT_MODULE_DEBUG
      */
  ...
  if(diff > 0)
  {
    ...
    #ifdef HEAT_MODULE_DEBUG
      if(!onMyWay) // чтобы не спамило в Serial4
      {
		  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
		  {
			  Serial4.print(channel);
			  Serial4.print(" Heater::open(), want to CLOSE for ms: ");
			  Serial4.println(timerCounter);
		  }
      }
    #endif // HEAT_MODULE_DEBUG  
  ...
  else
  {
    // текущая позиция меньше запрошенной, надо открыться
      ...
      if(!onMyWay) // чтобы не спамило в Serial4
      {
		  ...
		  {
			  Serial4.print(channel);
			  Serial4.print(" Heater::open(), want to OPEN for ms: ");
			  Serial4.println(timerCounter);
		  }
      }
    #endif // HEAT_MODULE_DEBUG
...
void LogicManageModuleClass::makeHeatDecision(Heater& heater, HeatSettings& section, HeatDriveCommand& decision, uint8_t& decisionPercents, float& lastI)
{
  ...
  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
  {
	  Serial4.print("Section temperature: ");
	  Serial4.println(sectionTemperature);
	  Serial4.print("Ambient temperature: ");
	  Serial4.println(ambientTemperature);
  }
  #endif // HEAT_MODULE_DEBUG
  ...
  if(!sectionTemperature.HasData())
  {
	  ...
	  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	  {
		  Serial4.println("NO SECTION TEMPERATURE!!!");
	  }
    #endif // HEAT_MODULE_DEBUG
    decision = hdHeatOff;
  ...
  if(formattedTemperature >= border)
  {
	  ...
	  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	  {
		  Serial4.println("SECTION OVERHEAT!!!");
	  }
    #endif // HEAT_MODULE_DEBUG
  ...
  if(formattedTemperature <= border)
  {
	...
	if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	{
		Serial4.println("SECTION FROSEN!!!");
		Serial4.print("New percents computed (SECTION FROZEN): ");
		Serial4.println(decisionPercents);
	}
    #endif // HEAT_MODULE_DEBUG
  ...
  if(formattedTemperature >= border)
  {
	  ...
	  if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	  {
		  Serial4.println("AMBIENT TEMPERATURE IS GOOD :)");
		  Serial4.print("Percents not changed (AMBIENT IS GOOD): ");
		  Serial4.println(decisionPercents);
	  }
    #endif // HEAT_MODULE_DEBUG
...
//  {
	...
	if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	{
		Serial4.println("NEED TO HEAT AMBIENT!");
		Serial4.print("New percents computed (HEAT AMBIENT): ");
		Serial4.println(decisionPercents);
	}
    #endif // HEAT_MODULE_DEBUG
...
{	
	...
	if (debug.HEAT_MODULE_DEBUG_K == HIGH)
	{
		Serial4.println("MAKE HEAT DECISION!");
	}
	#endif // HEAT_MODULE_DEBUG
...
/*
	...
	Serial4.print("Want temperature, sensor=");
...
/*
	#if defined(HEAT_MODULE_DEBUG)
	Serial4.print("Want temperature, sensorIndex=");
	Serial4.print(sensorIndex);
	Serial4.print(", module=");
	Serial4.println(moduleName);
	#endif
*/
...
void LogicManageModuleClass::Setup()
{
  ...
  #endif
	#ifdef USE_HUMIDITY_DRAINING_MODULE
  	ReloadHumidityDrainingSettings();
  #endif
}
//--------------------------------------------------------------------------------------------------------------------------------------
...
{ 
  ...
  if(!_can_work)
  {
	...
	if(windowAutoMode != lastWindowAutoMode)
	{
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.println(F("Windows mode changed, clear windows commands!"));
		}
  ...
  /*
   ...
   if(WindowModule->CanDriveWindows()) // только если можем управлять окнами
   {
  		...
  		if(wantToCloseWindowsBecauseOfHeat)
  		{
  			...
  			#ifdef WINDOW_MANAGE_DEBUG
  			//	Serial4.println(F("Want to close ALL windows because of HEAT ON!"));
  			#endif // WINDOW_MANAGE_DEBUG
...
void LogicManageModuleClass::makeWindowsSectionDecision(uint8_t sectionIndex, WManageChannel& section)
{
    ...
    if (!ourWindowsNumbers.size()) // нет ни одного окна по секции
    {
        ...
        if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
        {
            Serial4.print(F("Section #"));
            Serial4.print(sectionIndex);
            Serial4.println(F(" has no windows linked, can't make decision!"));
        }
#endif // WINDOW_MANAGE_DEBUG
    ...
    if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
    {
        Serial4.print(F("Windows for section #"));
        Serial4.print(sectionIndex);
        Serial4.print(F(" are: "));
        ...
        for (size_t i = 0; i < ourWindowsNumbers.size(); i++)
        {
            ...
            Serial4.print(ourWindowsNumbers[i]);
        }
        Serial4.println();
    }
	#endif // WINDOW_MANAGE_DEBUG
	...
	if(sectionTemperature.HasData())
	{
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" temperature is: "));
			Serial4.println(sectionTemperature);
		}
		#endif // WINDOW_MANAGE_DEBUG
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" histeresis are: "));
			Serial4.println(histeresis);
		}
		#endif // WINDOW_MANAGE_DEBUG
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" normalized temperature are: "));
			Serial4.println(normalizedTemperature);
		}
    #endif // WINDOW_MANAGE_DEBUG      
	...
	if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
	{
		Serial4.print(F("Section #"));
		Serial4.print(sectionIndex);
		Serial4.print(F(" lastTemperatureDecision are: "));
		Serial4.println(lastTemperatureDecision);
	}
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" t25percents are: "));
			Serial4.println(t25percents);

			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" t50percents are: "));
			Serial4.println(t50percents);

			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" t75percents are: "));
			Serial4.println(t75percents);

			Serial4.print(F("Section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" t100percents are: "));
			Serial4.println(t100percents);
		}
    #endif // WINDOW_MANAGE_DEBUG   
    ...
    */
	...
	if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
	{
		Serial4.print(F("Temperature decision for section #"));
		Serial4.print(sectionIndex);
		Serial4.print(F(" are: "));
		Serial4.println(temperatureDecision);
	}
		#endif
 ...
 // при этом направление ветра - будет неважно: раз закрыли по порогу и ориентации, то надо ждать снижения ветра.
  ...
  static Vector<size_t> previouslyClosedByHurricaneWindows; // список окон, закрытых по уставке урагана
	...
	if(windSpeedNow >= windSpeedHighBorder) // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
	{
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Wind speed border detected, close section #"));
			Serial4.print(sectionIndex);
			Serial4.print(F(" linked windows, windows are: "));
			...
			for (size_t i = 0; i < orientationCloseWindows.size(); i++)
			{
				if (i > 0)
					Serial4.print(F(", "));
				    Serial4.print(orientationCloseWindows[i]);
			}
			Serial4.println();
		}
		#endif // WINDOW_MANAGE_DEBUG
  ...
  if(windSpeedNow <= windSpeedLowBorder)
  {
	...
	if(windSpeedNow >= windSpeedHighBorder)  // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
	{
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
			Serial4.print(F("Hurricane detected, close ALL section #"));
			Serial4.print(sectionIndex);
			Serial4.println(F(" linked windows!"));
		}
		#endif // WINDOW_MANAGE_DEBUG
	...
	if(hasRain)
	{
		...
		if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
		{
            DEBUG_LOG(F("Rain detected, want to close windows in section #"));
            DEBUG_LOG(String(sectionIndex));
            DEBUG_LOG(F(", windows are: "));
			...
			for (size_t i = 0; i < rainLinkedWindows.size(); i++)
			{
                ...
                DEBUG_LOG(String(rainLinkedWindows[i]));
			}
            DEBUG_LOGLN("");
		}
		#endif // WINDOW_MANAGE_DEBUG
    ...
    if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
    {
        for (size_t i = 0; i < ourWindowsNumbers.size(); i++)
        {
            ...
            if (windowNumber < windowsDecision.size())
            {
                if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
                {
                    DEBUG_LOG(F("Decision for section #"));
                    DEBUG_LOG(String(sectionIndex));
                    DEBUG_LOG(F(" and window #"));
                    DEBUG_LOG(String(windowNumber));
                    DEBUG_LOG(F(" are: "));
                    DEBUG_LOGLN(String(windowsDecision[windowNumber]));
                }
            }
        } // for	
    }
...
void LogicManageModuleClass::makeWindowsDecision()
{
	...
	if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
	{
		Serial4.println("MAKE WINDOWS DECISION!");
	}
	#endif // WINDOW_MANAGE_DEBUG	
...
void LogicManageModuleClass::applyWindowsDecision()
{
	...
	for(size_t i=0;i<windowsDecision.size();i++)
	{
        ...
        if(window && !window->IsBusy() && !window->IsInUninterruptedWay())
        {
					...
					if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
					{
                        DEBUG_LOG(F("Send command: "));
                        DEBUG_LOGLN(String(command));
					}
      				#endif // WINDOW_MANAGE_DEBUG
...
void LogicManageModuleClass::firstCallSetup()
{
	...
	// настраиваем затенение при старте
		...
		rainSensor.setup();  // настраиваем датчик дождя
 		windSensor.setup();  // настраиваем датчик ветра
...
////////////////////////////////////////////////////////////////////////////   
	 ...
	 if (which == F("DRAINING"))
	 {
		 ...
		 if (argsCount > 1)
		 {
			 ...
			 if (param == STATE_ON) // CTSET=LOGIC|DRAINING|ON|channel (optional)
			 {
				 // включить распрыскивание по каналу
				 if (argsCount < 3) // все каналы
				 {
					 ...
					 draining1.switchToMode(hdrManual);
					 draining2.switchToMode(hdrManual);
					 draining3.switchToMode(hdrManual);
				 }
				 else
				 {
					 HumidityDraining* cv = getDraining(atoi(command.GetArg(2)));
					 if (cv)
					 {
						 cv->turn(true);
						 cv->switchToMode(hdrManual);
					 }
				 }
				 ...
				 if (param == STATE_OFF) // CTSET=LOGIC|DRAINING|OFF|channel (optional)
				 {
					 // выключить осушение по каналу
					 if (argsCount < 3) // все каналы
					 {
						 draining1.turn(false);
						 draining2.turn(false);
						 draining3.turn(false);
					 ...
					 if (param == WORK_MODE) // CTSET=LOGIC|DRAINING|MODE|(AUTO or MANUAL)
					 {
						 if (argsCount > 2)
						 {
							 String mode = command.GetArg(2);
							 ...
							 {
								 int channel = atoi(command.GetArg(3));
								 HumidityDraining* cv = channel == 0 ? &draining1 : channel == 1 ? &draining2 : &draining3;

								 if (mode == WM_AUTOMATIC)
									 cv->switchToMode(hdrAuto);
								 else
									 cv->switchToMode(hdrManual);
							 ...
							 {
								 if (mode == WM_AUTOMATIC) // CTSET=LOGIC|DRAINING|MODE|AUTO
								 {
									 draining1.switchToMode(hdrAuto);
									 draining2.switchToMode(hdrAuto);
									 draining3.switchToMode(hdrAuto);
						 ...
						 if (param == F("ACTIVE")) // вкл/выкл модуль канала осушения, CTSET=LOGIC|DRAINING|ACTIVE|num|active flag
						 {
							 if (argsCount > 3)
							 {
								 int channelNum = atoi(command.GetArg(2));
								 if (channelNum >= 0 && channelNum <= 2)
								 {
									 HumidityDraining* v = channelNum == 0 ? &draining1 : channelNum == 1 ? &draining2 : &draining3;
									 HumidityDrainingSettings vs = v->getSettings();
									 vs.active = atoi(command.GetArg(3));

									 MainController->GetSettings()->SetHumidityDrainingSettings(channelNum, vs);

									 // перезагружаем настройки осушения
									 ReloadHumidityDrainingSettings();
							 ...
							 if (param == F("CHANNEL")) // настройки канала осушения, СTSET=LOGIC|DRAINING|CHANNEL|num|active|sensorIndex|drainingOnValue|drainingOffValue|histeresis|startWorkTime|endWorkTime
							 {
								 if (argsCount > 9)
								 {
									 int channelNum = atoi(command.GetArg(2));
									 if (channelNum >= 0 && channelNum <= 2)
									 {
										 ...
										 vs.active = atoi(command.GetArg(3));
										 vs.sensorIndex = atoi(command.GetArg(4));
										 vs.drainingOnValue = atoi(command.GetArg(5));
										 vs.drainingOffValue = atoi(command.GetArg(6));
										 vs.histeresis = atoi(command.GetArg(7));
										 vs.startWorkTime = atoi(command.GetArg(8));
										 vs.endWorkTime = atoi(command.GetArg(9));
									 ...
									 if (argsCount > 4) // настройки канала осушения, СTSET=LOGIC|DRAINING|CHANNEL|num|drainingOnValue|drainingOffValue
									 {
										 int channelNum = atoi(command.GetArg(2));
										 if (channelNum >= 0 && channelNum <= 2)
										 {
											 ...
											 vs.drainingOnValue = atoi(command.GetArg(3));
											 vs.drainingOffValue = atoi(command.GetArg(4));
		  ...
		  if (which == F("DRAINING"))
			  {
				  ...
				  if (argsCount > 1)
				  {
					  ...
					  if (param == F("STATUS")) // CTGET=LOGIC|DRAINING|STATUS, returns OK=LOGIC|HSPRAY|STATUS|section1 on| section2 on|section3 on
					  {
						  PublishSingleton.Flags.Status = true;
						  PublishSingleton = which;
						  PublishSingleton << PARAM_DELIMITER << param;

						  PublishSingleton << PARAM_DELIMITER << draining1.isOn();
						  PublishSingleton << PARAM_DELIMITER << draining2.isOn();
						  PublishSingleton << PARAM_DELIMITER << draining3.isOn();

					  } //if(param == F("STATUS"))
					  else
						  if (param == F("STATE")) // CTGET=LOGIC|DRAINING|STATE, returns OK=LOGIC|HSPRAY|STATE|section1 on|section1 work mode|section2 on|section2 work mode|section3 on|section3 work mode
						  {
							  PublishSingleton.Flags.Status = true;
							  PublishSingleton = which;
							  PublishSingleton << PARAM_DELIMITER << param;
							  ...
							  PublishSingleton << PARAM_DELIMITER << draining1.isOn();
							  PublishSingleton << PARAM_DELIMITER << (draining1.getWorkMode() == hdrAuto ? F("AUTO") : F("MANUAL"));

							  PublishSingleton << PARAM_DELIMITER << draining2.isOn();
							  PublishSingleton << PARAM_DELIMITER << (draining2.getWorkMode() == hdrAuto ? F("AUTO") : F("MANUAL"));

							  PublishSingleton << PARAM_DELIMITER << draining3.isOn();
							  PublishSingleton << PARAM_DELIMITER << (draining3.getWorkMode() == hdrAuto ? F("AUTO") : F("MANUAL"));

						  }
						  else
							  if (param == F("ACTIVE")) // CTGET=LOGIC|DRAINING|ACTIVE, returns OK=LOGIC|HSPRAY|ACTIVE|section1 active| section2 active|section3 active
							  {
								  PublishSingleton.Flags.Status = true;
								  PublishSingleton = which;
								  PublishSingleton << PARAM_DELIMITER << param;

								  PublishSingleton << PARAM_DELIMITER << draining1.getSettings().active;
								  PublishSingleton << PARAM_DELIMITER << draining2.getSettings().active;
								  PublishSingleton << PARAM_DELIMITER << draining3.getSettings().active;

							  } //if(param == F("ACTIVE"))
							  else
								  if (param == F("CHANNEL")) // CTGET=LOGIC|DRAINING|CHANNEL|num, returns OK=LOGIC|HSPRAY|CHANNEL|num|active|sensorIndex|sprayOnValue|sprayOffValue|histeresis|startWorkTime|endWorkTime
								  {
									  if (argsCount > 2)
									  {
										  int channelNum = atoi(command.GetArg(2));
										  if (channelNum >= 0 && channelNum <= 2)
										  {
											  ...
											  PublishSingleton << PARAM_DELIMITER << vs.active
												  << PARAM_DELIMITER << vs.sensorIndex
												  << PARAM_DELIMITER << vs.drainingOnValue
												  << PARAM_DELIMITER << vs.drainingOffValue
												  << PARAM_DELIMITER << vs.histeresis
												  << PARAM_DELIMITER << vs.startWorkTime
												  << PARAM_DELIMITER << vs.endWorkTime
												  ;
...
void WindSensor::setup()
{
  ...
  //Serial.print("bnd.WorkMode: ");
  //Serial.println(bnd.WorkMode);
	...
	if(bnd.Pin != UNBINDED_PIN)
	{
		if(EEPROMSettingsModule::SafePin(bnd.Pin))
		{
			attachInterrupt(digitalPinToInterrupt(bnd.Pin),pulse,WIND_SENSOR_INT_LEVEL);  // Определяем скорость ветра по прерыванию
		}
	}

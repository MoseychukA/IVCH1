...
public:
	...
	bool isOn() { return onFlag; }
	HDRWorkMode getWorkMode() { return workMode; }

	// управление в ручном режиме
	void switchToMode(HDRWorkMode mode) { workMode = mode; machineState = hdrIdle; }
	void turn(bool isOn) { setState(isOn); machineState = hdrIdle; }

	HumidityDrainingSettings& getSettings() { return settings; }
//
private:
	HumidityDrainingSettings settings;
	uint8_t channel;
	bool onFlag;
	HDRMachineState machineState;

	HDRWorkMode workMode;
...
class LogicManageModuleClass : public AbstractModule
{
  private:
	...
	#ifdef USE_HUMIDITY_DRAINING_MODULE
		HumidityDraining draining1;
		HumidityDraining draining2;
		HumidityDraining draining3;
		void makeDrainingDecision();
	#endif
 ...
 uint8_t ComputePI(int32_t cT, int32_t wT, float& i, uint8_t kP, float pMin, float pMax, float kI, float iMin, float iMax, float kProp);
  ...
  public:
	...
	#ifdef USE_HUMIDITY_DRAINING_MODULE
		void ReloadHumidityDrainingSettings();
		HumidityDraining* getDraining(uint8_t channel) { if (channel == 1) return &draining2; else if (channel == 2) return &draining3; return &draining1; }
	#endif

...
#define linkPortExpander    2 // привязка к расширителю портов 
//PortExpander
...
} HumidityDrainingBinding; // настройки железа для модуля осушения
#pragma pack(pop)
//--------------------------------------------------------------------------------------------------------------------------------------
#pragma pack(push,1)
typedef struct
...
typedef struct
{
	...
	uint8_t ID_Misol;        // ID метеостанции Misol
...
typedef struct
{
	...
	uint8_t DisplayType;            // тип дисплея (0 - не используется, 1 - 7 дюймов SSD1963, 2 - 4 дюйма OTM8009A, 3 - 4 дюйма OTM8009A)
...
typedef struct
{
	uint8_t WIFI_DEBUG_K;                // режим отладки Wi-Fi-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t GSM_DEBUG_MODE_K;            // режим отладки GSM-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t LOGGING_DEBUG_MODE_K;        // режим отладки модуля логгирования информации (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t WATER_DEBUG_K;               // отладочный режим модуля полива (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t NRF_DEBUG_K;                 // отладочный режим nRF, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t LORA_DEBUG_K;                // отладочный режим LoRa, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t RS485_DEBUG_K;               // отладочный режим RS-485, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t UNI_DEBUG_K;                 // отладочный режим универсальных модулей (КОНФИГУРАТОР НЕ ЗАПУСКАТЬ, ТОЛЬКО МОНИТОР ПОРТА!)
	uint8_t PH_DEBUG_K;                  // Режим отладки модуля pH (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t IOT_DEBUG_K;                 // Режим отладки модуля IOT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t HTTP_DEBUG_K;                // отладочный режим провайдеров HTTP-запросов, как правило - надо включать совместно с WIFI_DEBUG (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t MQTT_DEBUG_K;                // отладочный режим MQTT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t SCENE_DEBUG_K;               // отладочный режим модуля сценариев
	uint8_t WINDOW_MANAGE_DEBUG_K;       // закомментировать для выключения отладочной информации по управлению окнами
	uint8_t HEAT_MODULE_DEBUG_K;         // закомментировать для выключения отладочной информации по отоплению
	uint8_t SHADOW_MODULE_DEBUG_K;       // закомментировать для выключения отладочной информации по затенению
	uint8_t DEBUG_MEASURE_UPDATE_TIME_K; // закомментировать для выключения проверки быстродействия (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t CYCLE_VENT_DEBUG_K;          // отладочный режим модуля циклической вентиляции
	uint8_t VENT_DEBUG_K;                // отладочный режим модуля вентиляции по температуре
	uint8_t DS18B20_DEBUG_K;             // отладочный режим опроса DS18B20
	uint8_t THERMOSTAT_DEBUG_K;          // отладочный режим модуля термостатов
	uint8_t POWER_DEBUG_K;               // отладочный режим управления питанием
	uint8_t HUMIDITY_SPRAY_DEBUG_K;      // отладочный режим управление распрыскиванием для поддержания влажности
	uint8_t WATER_TANK_MODULE_DEBUG_K;   // отладочный режим модуля контроля уровня воды, закомментировать, если в нём нет необходимости(КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	uint8_t MISOL_DEBUG_K;               // отладочный режим модуля метеостанции Misol
	uint8_t SOIL_MOISTURE_MODULE_DEBUG_K;// отладочный режим датчиков влажности почвы
	uint8_t ONE_WIRE_EMULATION_MODULE_DEBUG_K;  // отладочный режим эмулятора 1 Ware
	uint8_t DRAINING_DEBUG_K;            // отладочный режим модуля осушения     
	uint8_t MCP_DEBUG_K;                 // отладочный режимрасширителя портов
...
private:
	...
	SysDebug sysDebug;
	HumidityDrainingBinding drainingBinding[3];
...
public:
	...
	// возвращает настройки привязки модуля осушения
	HumidityDrainingBinding& GetHumidityDrainingBinding(uint8_t channel) { return drainingBinding[channel]; }
	...
	// возвращает настройки модуля вывода отладочной информации о работе модулей контроллера
	SysDebug& GetSysDebug() { return sysDebug; }

};
//--------------------------------------------------------------------------------------------------------------------------------------
extern EEPROMSettingsModule* HardwareBinding;
extern SysDebug* sysdebug;

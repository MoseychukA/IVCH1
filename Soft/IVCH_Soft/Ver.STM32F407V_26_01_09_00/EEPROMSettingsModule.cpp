...
void EEPROMSettingsModule::Setup()
{
  ...
  if(!read(WIND_BINDING_ADDRESS, windSensorBinding))
  {
	  ...
	  windSensorBinding.ID_Misol = 255;
  }  
  ...
  for(uint8_t i=0;i<3;i++)
  {
   ...
   // читаем настройки осушения
  memset(drainingBinding, 0, sizeof(drainingBinding));
  for (uint8_t i = 0; i < 3; i++)
  {
	  int addr = DRAINING_BINDING_ADDRESS + i * (sizeof(HumidityDrainingBinding) + 3);  // 2 байта - заголовок, 1 - CRC8
	  if (!read(addr, drainingBinding[i]))
	  {
		  memset(&(drainingBinding[i]), 0, sizeof(HumidityDrainingBinding));

		  drainingBinding[i].LinkType = linkUnbinded; // нет привязки
		  drainingBinding[i].Pin = UNBINDED_PIN; // нет привязки
		  drainingBinding[i].Level = LOW;
	  }
   ...
   // читаем настройки режима DEBUG
   memset(&sysDebug, 0, sizeof(sysDebug));   // 
   if (!read(DEBUG_BINDING_ADDRESS, sysDebug))
   {
	   memset(&sysDebug, 0, sizeof(sysDebug));
	
	   sysDebug.WIFI_DEBUG_K = LOW;                // режим отладки Wi-Fi-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.GSM_DEBUG_MODE_K = LOW;            // режим отладки GSM-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.LOGGING_DEBUG_MODE_K = LOW;        // режим отладки модуля логгирования информации (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.WATER_DEBUG_K = LOW;               // отладочный режим модуля полива (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.NRF_DEBUG_K = LOW;                 // отладочный режим nRF, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.LORA_DEBUG_K = LOW;                // отладочный режим LoRa, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.RS485_DEBUG_K = LOW;               // отладочный режим RS-485, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.UNI_DEBUG_K = LOW;                 // отладочный режим универсальных модулей (КОНФИГУРАТОР НЕ ЗАПУСКАТЬ, ТОЛЬКО МОНИТОР ПОРТА!)
	   sysDebug.PH_DEBUG_K = LOW;                  // Режим отладки модуля pH (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.IOT_DEBUG_K = LOW;                 // Режим отладки модуля IOT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.HTTP_DEBUG_K = LOW;                // отладочный режим провайдеров HTTP-запросов, как правило - надо включать совместно с WIFI_DEBUG (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.MQTT_DEBUG_K = LOW;                // отладочный режим MQTT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.SCENE_DEBUG_K = LOW;               // отладочный режим модуля сценариев
	   sysDebug.WINDOW_MANAGE_DEBUG_K = LOW;       // закомментировать для выключения отладочной информации по управлению окнами
	   sysDebug.HEAT_MODULE_DEBUG_K = LOW;         // закомментировать для выключения отладочной информации по отоплению
	   sysDebug.SHADOW_MODULE_DEBUG_K = LOW;       // закомментировать для выключения отладочной информации по затенению
	   sysDebug.DEBUG_MEASURE_UPDATE_TIME_K = LOW; // закомментировать для выключения проверки быстродействия (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.CYCLE_VENT_DEBUG_K = LOW;          // отладочный режим модуля циклической вентиляции
	   sysDebug.VENT_DEBUG_K = LOW;                // отладочный режим модуля вентиляции по температуре
	   sysDebug.DS18B20_DEBUG_K = LOW;             // отладочный режим опроса DS18B20
	   sysDebug.THERMOSTAT_DEBUG_K = LOW;          // отладочный режим модуля термостатов
	   sysDebug.POWER_DEBUG_K = LOW;               // отладочный режим управления питанием
	   sysDebug.HUMIDITY_SPRAY_DEBUG_K = LOW;      // отладочный режим управление распрыскиванием для поддержания влажности
	   sysDebug.WATER_TANK_MODULE_DEBUG_K = LOW;   // отладочный режим модуля контроля уровня воды, закомментировать, если в нём нет необходимости(КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
	   sysDebug.MISOL_DEBUG_K = LOW;               // отладочный режим модуля метеостанции Misol
	   sysDebug.SOIL_MOISTURE_MODULE_DEBUG_K = LOW;// отладочный режим датчиков влажности почвы
	   sysDebug.ONE_WIRE_EMULATION_MODULE_DEBUG_K = LOW;  // отладочный режим эмулятора 1 Ware
	   sysDebug.DRAINING_DEBUG_K = LOW;            // отладочный режим модуля осушения    
       sysDebug.MCP_DEBUG_K = LOW;                 // отладочный режим расширителя портов
   }
     ...
     else
     {
       ...
       if(param == F("WTANK")) // установить настройки модуля контроля бака с водой: CTSET=EES|WTANK|link type|level|max work time|empty_cm|full_cm
       {
          ...
          else
          {
			 ...
			 Serial4.print(F("LinkType = "));
			 Serial4.println(waterTankBinding.LinkType);
			 Serial4.print(F("Level = "));
			 Serial4.println(waterTankBinding.Level);
			 Serial4.print(F("MaxWorkTime = "));
			 Serial4.println(waterTankBinding.MaxWorkTime);
			 Serial4.print(F("DistanceEmpty = "));
			 Serial4.println(waterTankBinding.DistanceEmpty);
			 Serial4.print(F("DistanceFull = "));
			 Serial4.println(waterTankBinding.DistanceFull);
#endif 
          }
       } // F("WTANK")       
       #endif // USE_WATER_TANK_MODULE      
...
/////////////////////////////////////////////////////
		   ...
		   if (param == F("DRAINING")) // установить настройки осушения: CTSET=EES|DRAINING|link type 1|mcp address 1|pin 1|level 1|link type 2|mcp address 2|pin 2|level 2|link type 3|mcp address 3|pin 3|level 3
		   {
			   if (argsCnt < 13)
			   {
				   if (wantAnswer)
			   ...
			   {
				   // аргументов хватает
				   drainingBinding[0].LinkType = atoi(command.GetArg(1));
				   drainingBinding[0].MCPAddress = atoi(command.GetArg(2));
				   drainingBinding[0].Pin = atoi(command.GetArg(3));
				   drainingBinding[0].Level = atoi(command.GetArg(4));

				   drainingBinding[1].LinkType = atoi(command.GetArg(5));
				   drainingBinding[1].MCPAddress = atoi(command.GetArg(6));
				   drainingBinding[1].Pin = atoi(command.GetArg(7));
				   drainingBinding[1].Level = atoi(command.GetArg(8));

				   drainingBinding[2].LinkType = atoi(command.GetArg(9));
				   drainingBinding[2].MCPAddress = atoi(command.GetArg(10));
				   drainingBinding[2].Pin = atoi(command.GetArg(11));
				   drainingBinding[2].Level = atoi(command.GetArg(12));


				   for (uint8_t i = 0; i < 3; i++)
				   {
					   int addr = DRAINING_BINDING_ADDRESS + i * (sizeof(HumidityDrainingBinding) + 3); // 2 байта - заголовок, 1 - CRC8
					   write(addr, drainingBinding[i]);
				   }
...
#endif // #ifdef USE_HUMIDITY_DRAINING_MODULE
///////////////////////////////////////////////////   
       ...
       if(param == F("WIND")) // установить настройки датчика ветра: CTSET=EES|WIND|pin|dir pin|speed coeff|east from|east to|south from|south to|west from|west to|north from|north to|histeresis|num samples|work mode|ID Misol
       {
          if(argsCnt < 16)
          {
            if(wantAnswer)
          ...
          else
          {
			 ...
			 windSensorBinding.ID_Misol = atoi(command.GetArg(++cntr));
	
//
//			 if(sysDebug.MISOL_DEBUG_K == HIGH)
//			 {
////#ifdef MISOL_DEBUG
//				 Serial4.print("ID_Misol receive -");
//				 Serial4.println(windSensorBinding.ID_Misol);
////#endif
//			 }
       ...
       if(param == F("TFTS")) // установить настройки датчика на TFT-экране: CTSET=EES|TFTS|rec index|type|index|caption
       {
		...
		if (param == F("DEBUG")) // установить настройки DEBUG
		{
			if (argsCnt < sizeof(sysDebug))   // Количество флагов DEBUG
			{
				if (wantAnswer)
				{
					PublishSingleton = PARAMS_MISSED;
			...
			else
			{
				...
				sysDebug.WIFI_DEBUG_K = atoi(command.GetArg(++cntr));                // 1 режим отладки Wi-Fi-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.GSM_DEBUG_MODE_K = atoi(command.GetArg(++cntr));            // 2 режим отладки GSM-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.LOGGING_DEBUG_MODE_K = atoi(command.GetArg(++cntr));        // 3 режим отладки модуля логгирования информации (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.WATER_DEBUG_K = atoi(command.GetArg(++cntr));               // 4 отладочный режим модуля полива (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.NRF_DEBUG_K = atoi(command.GetArg(++cntr));                 // 5 отладочный режим nRF, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.LORA_DEBUG_K = atoi(command.GetArg(++cntr));                // 6 отладочный режим LoRa, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.RS485_DEBUG_K = atoi(command.GetArg(++cntr));               // 7 отладочный режим RS-485, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.UNI_DEBUG_K = atoi(command.GetArg(++cntr));                 // 8 отладочный режим универсальных модулей (КОНФИГУРАТОР НЕ ЗАПУСКАТЬ, ТОЛЬКО МОНИТОР ПОРТА!)
				sysDebug.PH_DEBUG_K = atoi(command.GetArg(++cntr));                  // 9 Режим отладки модуля pH (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.IOT_DEBUG_K = atoi(command.GetArg(++cntr));                 // 10 Режим отладки модуля IOT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.HTTP_DEBUG_K = atoi(command.GetArg(++cntr));                // 11 отладочный режим провайдеров HTTP-запросов, как правило - надо включать совместно с WIFI_DEBUG (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.MQTT_DEBUG_K = atoi(command.GetArg(++cntr));                // 12 отладочный режим MQTT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.SCENE_DEBUG_K = atoi(command.GetArg(++cntr));               // 13 отладочный режим модуля сценариев
				sysDebug.WINDOW_MANAGE_DEBUG_K = atoi(command.GetArg(++cntr));       // 14 закомментировать для выключения отладочной информации по управлению окнами
				sysDebug.HEAT_MODULE_DEBUG_K = atoi(command.GetArg(++cntr));         // 15 закомментировать для выключения отладочной информации по отоплению
				sysDebug.SHADOW_MODULE_DEBUG_K = atoi(command.GetArg(++cntr));       // 16 закомментировать для выключения отладочной информации по затенению
				sysDebug.DEBUG_MEASURE_UPDATE_TIME_K = atoi(command.GetArg(++cntr)); // 17 закомментировать для выключения проверки быстродействия (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.CYCLE_VENT_DEBUG_K = atoi(command.GetArg(++cntr));          // 18 отладочный режим модуля циклической вентиляции
				sysDebug.VENT_DEBUG_K = atoi(command.GetArg(++cntr));                // 19 отладочный режим модуля вентиляции по температуре
				sysDebug.DS18B20_DEBUG_K = atoi(command.GetArg(++cntr));             // 20 отладочный режим опроса DS18B20
				sysDebug.THERMOSTAT_DEBUG_K = atoi(command.GetArg(++cntr));          // 21 отладочный режим модуля термостатов
				sysDebug.POWER_DEBUG_K = atoi(command.GetArg(++cntr));               // 22 отладочный режим управления питанием
				sysDebug.HUMIDITY_SPRAY_DEBUG_K = atoi(command.GetArg(++cntr));      // 23 отладочный режим управление распрыскиванием для поддержания влажности
				sysDebug.WATER_TANK_MODULE_DEBUG_K = atoi(command.GetArg(++cntr));   // 24 отладочный режим модуля контроля уровня воды, закомментировать, если в нём нет необходимости(КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
				sysDebug.MISOL_DEBUG_K = atoi(command.GetArg(++cntr));               // 25 отладочный режим модуля метеостанции Misol
				sysDebug.SOIL_MOISTURE_MODULE_DEBUG_K = atoi(command.GetArg(++cntr));// 26 отладочный режим датчиков влажности почвы
				sysDebug.ONE_WIRE_EMULATION_MODULE_DEBUG_K = atoi(command.GetArg(++cntr));  // 27 отладочный режим эмулятора 1 Ware
				sysDebug.DRAINING_DEBUG_K = atoi(command.GetArg(++cntr));            // 28 отладочный режим модуля осушения  
                sysDebug.MCP_DEBUG_K = atoi(command.GetArg(++cntr));                 // 29 отладочный режим расширителя портов 

                write(DEBUG_BINDING_ADDRESS, sysDebug);

				//Serial4.print("MCP_DEBUG_K - ");
				//Serial4.println(sysDebug.MCP_DEBUG_K);
...
///////////////////////////////////////////////////////////////
		...
		if (param == F("DRAINING")) // запросили настройки опрыскивания: CTGET=EES|HSPRAY
		{
			PublishSingleton.Flags.Status = true;
			PublishSingleton = param;
			PublishSingleton << PARAM_DELIMITER << drainingBinding[0].LinkType
				<< PARAM_DELIMITER << drainingBinding[0].MCPAddress
				<< PARAM_DELIMITER << drainingBinding[0].Pin
				<< PARAM_DELIMITER << drainingBinding[0].Level

				<< PARAM_DELIMITER << drainingBinding[1].LinkType
				<< PARAM_DELIMITER << drainingBinding[1].MCPAddress
				<< PARAM_DELIMITER << drainingBinding[1].Pin
				<< PARAM_DELIMITER << drainingBinding[1].Level

				<< PARAM_DELIMITER << drainingBinding[2].LinkType
				<< PARAM_DELIMITER << drainingBinding[2].MCPAddress
				<< PARAM_DELIMITER << drainingBinding[2].Pin
				<< PARAM_DELIMITER << drainingBinding[2].Level
				;
		} // F("DRAINING")
#endif // #ifdef USE_HUMIDITY_DRAINING_MODULE
////////////////////////////////////////////////////////////
        ...
        if(param == F("WIND")) // запросили настройки датчика ветра: CTGET=EES|WIND
        {
		  ...
		  << PARAM_DELIMITER << windSensorBinding.ID_Misol
          ;                  
        } // F("WIND")
        #endif // #ifdef USE_WIND_SENSOR
        #if defined(USE_BUZZER) && !defined(DISABLE_BUZZER_CONFIGURE)
		...
		if (param == F("DEBUG")) // запросили настройки DEBUG: CTGET=EES|DEBUG
		{
			PublishSingleton.Flags.Status = true;
			PublishSingleton = param;
			PublishSingleton << PARAM_DELIMITER << sysDebug.WIFI_DEBUG_K    // режим отладки Wi-Fi-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.GSM_DEBUG_MODE_K                 // режим отладки GSM-модуля (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.LOGGING_DEBUG_MODE_K             // режим отладки модуля логгирования информации (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.WATER_DEBUG_K                    // отладочный режим модуля полива (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.NRF_DEBUG_K                      // отладочный режим nRF, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.LORA_DEBUG_K                     // отладочный режим LoRa, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.RS485_DEBUG_K                    // отладочный режим RS-485, (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.UNI_DEBUG_K                      // отладочный режим универсальных модулей (КОНФИГУРАТОР НЕ ЗАПУСКАТЬ, ТОЛЬКО МОНИТОР ПОРТА!)
			<< PARAM_DELIMITER << sysDebug.PH_DEBUG_K                       // Режим отладки модуля pH (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.IOT_DEBUG_K                      // Режим отладки модуля IOT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.HTTP_DEBUG_K                     // отладочный режим провайдеров HTTP-запросов, как правило - надо включать совместно с WIFI_DEBUG (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.MQTT_DEBUG_K                     // отладочный режим MQTT (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.SCENE_DEBUG_K                    // отладочный режим модуля сценариев
			<< PARAM_DELIMITER << sysDebug.WINDOW_MANAGE_DEBUG_K            // закомментировать для выключения отладочной информации по управлению окнами
			<< PARAM_DELIMITER << sysDebug.HEAT_MODULE_DEBUG_K              // закомментировать для выключения отладочной информации по отоплению
			<< PARAM_DELIMITER << sysDebug.SHADOW_MODULE_DEBUG_K            // закомментировать для выключения отладочной информации по затенению
			<< PARAM_DELIMITER << sysDebug.DEBUG_MEASURE_UPDATE_TIME_K      // закомментировать для выключения проверки быстродействия (КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.CYCLE_VENT_DEBUG_K               // отладочный режим модуля циклической вентиляции
			<< PARAM_DELIMITER << sysDebug.VENT_DEBUG_K                     // отладочный режим модуля вентиляции по температуре
			<< PARAM_DELIMITER << sysDebug.DS18B20_DEBUG_K                  // отладочный режим опроса DS18B20
			<< PARAM_DELIMITER << sysDebug.THERMOSTAT_DEBUG_K               // отладочный режим модуля термостатов
			<< PARAM_DELIMITER << sysDebug.POWER_DEBUG_K                    // отладочный режим управления питанием
			<< PARAM_DELIMITER << sysDebug.HUMIDITY_SPRAY_DEBUG_K           // отладочный режим управление распрыскиванием для поддержания влажности
			<< PARAM_DELIMITER << sysDebug.WATER_TANK_MODULE_DEBUG_K        // отладочный режим модуля контроля уровня воды, закомментировать, если в нём нет необходимости(КОНФИГУРАТОР РАЗРЕШЕНО ЗАПУСКАТЬ, ПОРТ ОТЛАДОЧНОЙ ИНФОРМАЦИИ NATIVE (USB))
			<< PARAM_DELIMITER << sysDebug.MISOL_DEBUG_K                    // отладочный режим модуля метеостанции Misol
			<< PARAM_DELIMITER << sysDebug.SOIL_MOISTURE_MODULE_DEBUG_K     // отладочный режим датчиков влажности почвы
			<< PARAM_DELIMITER << sysDebug.ONE_WIRE_EMULATION_MODULE_DEBUG_K  // отладочный режим эмулятора 1 Ware
			<< PARAM_DELIMITER << sysDebug.DRAINING_DEBUG_K                 // отладочный режим модуля осушения  
            << PARAM_DELIMITER << sysDebug.MCP_DEBUG_K                      // отладочный режим расширителей портов 
				//29
			;

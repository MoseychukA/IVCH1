...
const char* AlertRule::GetTargetCommand()
{
  ...
  switch(Settings.TargetCommandType)
  {
    ...
    case commandMCPPinOn: // CTSET=MCP|WRITE|mcpNumber|mcpChannel|ON
    {
    ...
    case commandMCPPinOff: // CTSET=MCP|WRITE|mcpNumber|mcpChannel|OFF
    {
   ...
   if(strlen(SD_BUFFER) > 0)
   {
    ...
    // начинаем искать известную нам команду.
        ...
        if(!strcmp_P(tcModuleName,(const char*)F("MCP"))) // чего-то делаем с MCP
        {
                 ...
                 if (!strcmp_P(command.GetArg(argsCnt - 1), (const char*)STATE_ON))
                     Settings.TargetCommandType = commandMCPPinOn;
                 else
                     Settings.TargetCommandType = commandMCPPinOff;
        }
...
//--------------------------------------------------------------------------------------------------------------------------------------
uint8_t AlertModule::read8(uint32_t address, uint8_t defaultVal)
{
...
//--------------------------------------------------------------------------------------------------------------------------------------
String AlertModule::readString(uint32_t address, byte maxlength)
{
    String result;
...
void AlertModule::writeString(uint32_t address, const String& v, byte maxlength)
{

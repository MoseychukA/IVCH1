...
void WorkStatus::InitPCF8575Extenders()
{
    byte mcp_addresses[] = { PCF8575_ADDRESSES };

#if defined MCP_DEBUG
     Serial4.println("mcp_addresses");
#endif    

    for (byte i = 0; i < COUNT_OF_PCF8575_EXTENDERS; i++)
    {
#if defined MCP_DEBUG
         Serial4.println(mcp_addresses[i]);
#endif  
        Adafruit_PCF8575* bank = new Adafruit_PCF8575;
        bank->begin(mcp_addresses[i]);
        PCF8575Extenders[i] = bank;
    }
#if defined MCP_DEBUG
     Serial4.println("mcp_addresses End");
#endif  
}

//--------------------------------------------------------------------------------------------------------------------------------
Adafruit_PCF8575* WorkStatus::GetPCF8575_ByAddress(byte addr)
{
    for (byte i = 0; i < COUNT_OF_PCF8575_EXTENDERS; i++)
    {
        Adafruit_PCF8575* bank = PCF8575Extenders[i];
        if (bank->getAddress() == addr)
            return bank;
    }
...
//--------------------------------------------------------------------------------------------------------------------------------
void WorkStatus::PCF8575_PinMode(byte mcpAddress, byte mpcChannel, byte mode)
{
    Adafruit_PCF8575* bank = GetPCF8575_ByAddress(mcpAddress);
    if (!bank || mpcChannel > 15)
        return;
#if defined MCP_DEBUG
     Serial4.print("mcpAddress ");
     Serial4.print(mcpAddress);
     Serial4.print(" mpcChannel ");
     Serial4.print(mpcChannel);
     Serial4.print(" mode ");
...
//--------------------------------------------------------------------------------------------------------------------------------
void WorkStatus::PCF8575_PinWrite(byte mcpAddress, byte mpcChannel, byte level)
{
    Adafruit_PCF8575* bank = GetPCF8575_ByAddress(mcpAddress);
    if (!bank || mpcChannel > 15)
        return;
...
//--------------------------------------------------------------------------------------------------------------------------------
byte WorkStatus::PCF8575_PinRead(byte mcpAddress, byte mpcChannel)
{
    Adafruit_PCF8575* bank = GetPCF8575_ByAddress(mcpAddress);
    if (!bank || mpcChannel > 15)
        return 0xFF;
  ...
  // по обратной связи канала окна не был обработан
    ...
    if(!flags.isAnyWindowsFeedbackReceived) // если не получили ни одного пакета 
    {
      if(flags.isFirstCallOfWindowsFeedback) // и первый раз вызвали эту функцию
      {
        ...
        #ifdef USE_TEMP_SENSORS
          //Serial4.println(F("No feedback, close windows..."));
...
#if  defined(_DEBUG) || defined(WIFI_DEBUG) || defined(GSM_DEBUG_MODE) || defined(LOGGING_DEBUG_MODE) || defined(WATER_DEBUG) || defined(NRF_DEBUG) || defined(LORA_DEBUG) || defined(RS485_DEBUG) || defined(UNI_DEBUG) || defined(PH_DEBUG) || defined(IOT_DEBUG) || defined(HTTP_DEBUG) || defined(MQTT_DEBUG) || defined(SCENE_DEBUG) ||defined(WINDOW_MANAGE_DEBUG) || defined(HEAT_MODULE_DEBUG) || defined(SHADOW_MODULE_DEBUG) || defined(DEBUG_MEASURE_UPDATE_TIME) || defined(CYCLE_VENT_DEBUG) || defined(VENT_DEBUG) || defined(DS18B20_DEBUG) || defined(THERMOSTAT_DEBUG) || defined(POWER_DEBUG) || defined(HUMIDITY_SPRAY_DEBUG) || defined(WATER_TANK_MODULE_DEBUG) || defined(MISOL_DEBUG) || defined(SOIL_MOISTURE_MODULE_DEBUG) || defined(ONE_WIRE_EMULATION_MODULE_DEBUG) || defined(DRAINING_DEBUG) || defined(MCP_DEBUG)

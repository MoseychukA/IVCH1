      ...
      for(size_t i=0;i<sizeof(bnd.LampRelays)/sizeof(bnd.LampRelays[0]);i++)
      {
          ...
          {              
              ...
              if(bnd.LinkType == linkPortExpander)
              {
                  ...
                  WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress,bnd.LampRelays[i],OUTPUT);
                  WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress,bnd.LampRelays[i],!bnd.Level);
      ...
      for(size_t i=0;i<sizeof(bnd.LampRelays)/sizeof(bnd.LampRelays[0]);i++)
      {
          if(bnd.LampRelays[i] != UNBINDED_PIN)
          {
              ...
              if(bnd.LinkType == linkPortExpander)
              {
                 ...
                 WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress,bnd.LampRelays[i],flags.bRelaysIsOn ? bnd.Level : !bnd.Level);

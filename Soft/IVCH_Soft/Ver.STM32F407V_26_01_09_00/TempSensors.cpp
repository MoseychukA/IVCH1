...
void WindowState::Setup(uint8_t index, uint8_t relayChannel1, uint8_t relayChannel2)
{
  ...
  // настраиваем концевики
    ...
    if(ebnd.LinkType != linkUnbinded)
    {
          ...
          if(ebnd.LinkType == linkPortExpander)
          {
               ...
               if(ebnd.EndstopOpenPins[flags.Index] != UNBINDED_PIN)
               {
                  WORK_STATUS.PCF8575_PinMode(ebnd.MCPAddress1,ebnd.EndstopOpenPins[flags.Index],INPUT);
               }
               if(ebnd.EndstopClosePins[flags.Index] != UNBINDED_PIN)
               {
                  WORK_STATUS.PCF8575_PinMode(ebnd.MCPAddress2,ebnd.EndstopClosePins[flags.Index],INPUT);                
               }
            #endif
          }
...
bool WindowState::ChangePosition(uint32_t newPos, bool waitFor)
{
 ...
 if(IsBusy() && waitForChangePositionDone)
 {
  ...
  //  Serial4.println("Position  not reached!");
   return false;
 }
        ...
        CurrentPosition += dt;
            ...
            if(ebnd.LinkType != linkUnbinded)
            {
                ...
                if(ebnd.LinkType == linkPortExpander)
                {
                    ...
                    if(ebnd.EndstopOpenPins[flags.Index] != UNBINDED_PIN)
                    {
                      bAnyEndstopTriggered = WORK_STATUS.PCF8575_PinRead(ebnd.MCPAddress1,ebnd.EndstopOpenPins[flags.Index]) == ebnd.Level;
                    }
                  #endif
                }
        ...
        CurrentPosition -= dt;
            ...
            if(ebnd.LinkType != linkUnbinded)
            {
                ...
                if(ebnd.LinkType == linkPortExpander)
                {
                    ...
                    if(ebnd.EndstopClosePins[flags.Index] != UNBINDED_PIN)
                    {
                      bAnyEndstopTriggered = WORK_STATUS.PCF8575_PinRead(ebnd.MCPAddress2,ebnd.EndstopClosePins[flags.Index]) == ebnd.Level;
                    }
                  #endif
                }
  ...
  SysDebug debug = HardwareBinding->GetSysDebug();
   ...
   {        
        ...
        if(bnd.LinkType != linkUnbinded)
        {
                ...
                for(uint8_t kk=0;kk<8;kk++)
                {
                    if(bnd.WindowsPins[iter] != UNBINDED_PIN)
                    {
                        ...
                        if(bnd.LinkType == linkPortExpander)
                        {
                          ...
                          #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
                              WORK_STATUS.PCF8575_PinWrite(addr, bnd.WindowsPins[iter], bitRead(bVal,kk) );
                          #endif
                        }
                    } // if(bnd.WindowsPins[iter] != UNBINDED_PIN)
          ...
          } // if(bnd.LinkType == linkPortExpander || bnd.LinkType == linkMCP23017)
         } // if(bnd.LinkType != linkUnbinded)
   } // if(shiftRegisterDataSize > 0)
...
void TempSensors::SaveChannelState(uint8_t channel, uint8_t state)
{
  ...
  if(bnd.LinkType == linkPortExpander)
  {
    //Сохраняем состояние каналов для MCP
      ...
      if(bnd.LinkType != linkUnbinded)
      {
          if(bnd.LinkType == linkPortExpander)
          {
			...
			if (debug.WINDOW_MANAGE_DEBUG_K == HIGH)
			{
              ...
              if(bnd.LinkType == linkPortExpander) // MCP23S17
              {
                #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
                  ...
                  if(pin2 != UNBINDED_PIN)
                  {
                    ...
                    if(pin1 != UNBINDED_PIN)
                    {
                      ...
                      WORK_STATUS.PCF8575_PinMode(addr, pin1, OUTPUT);
                      WORK_STATUS.PCF8575_PinWrite(addr, pin1, !bnd.Level);
                    }
                    ...
                    if(pin2 != UNBINDED_PIN)
                    {
                      ...
                      WORK_STATUS.PCF8575_PinMode(addr, pin2, OUTPUT);
                      WORK_STATUS.PCF8575_PinWrite(addr, pin2, !bnd.Level);
                    }
                  #endif
              }
          }
...
void TempSensors::Setup()
{
  ...
  if(dbnd.LinkType != linkUnbinded)
  {
    ...
    if(dbnd.LinkType == linkPortExpander)
    {
      #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
  ...
  smallSensorsChange = 0;
   ...
   if(wbnd.LinkType == linkPortExpander)
   {
   ...
   } // if(wbnd.LinkType == linkPortExpander || wbnd.LinkType == linkMCP23017|| wbnd.LinkType == linkPCF8575)

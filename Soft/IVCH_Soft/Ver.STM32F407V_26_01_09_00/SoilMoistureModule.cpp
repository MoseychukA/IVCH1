...
void SoilMoistureModule::Setup()
{
  ...
  if(sbnd.LinkType != linkUnbinded && sbnd.PowerPin != UNBINDED_PIN)
  {
      ...
      if(sbnd.LinkType == linkPortExpander)
      {
              ...
              WORK_STATUS.PCF8575_PinMode(sbnd.MCPAddress,sbnd.PowerPin,OUTPUT);
              WORK_STATUS.PCF8575_PinWrite(sbnd.MCPAddress,sbnd.PowerPin,!sbnd.Level);
        #endif
      }
      ...
      for(size_t i=0;i<sizeof(sbnd.SensorType)/sizeof(sbnd.SensorType[0]);i++)
      {
        ...
        switch(type)
        {
          ...
          case FREQUENCY_SOIL_MOISTURE: // частотный датчик влажности почвы
          {
			  ...
			  if (debug.SOIL_MOISTURE_MODULE_DEBUG_K == HIGH)
			  {
				   Serial4.println("Update frequency sensors...");
			  }
            #endif
            ...
            if(!highTime) // ALWAYS HIGH,  BUS ERROR
            {
				...
				if (debug.SOIL_MOISTURE_MODULE_DEBUG_K == HIGH)
				{
					 Serial4.println("BUS ERROR, NO HIGH TIME");
				}
                #endif
                State.UpdateState(StateSoilMoisture,sensorCntr,(void*)&h);
                sensorCntr++;
            ...
            else
            {
              ...
              if(!lowTime || !highTime)
              {
				  ...
				  if (debug.SOIL_MOISTURE_MODULE_DEBUG_K == HIGH)
				  {
					   Serial4.println("BUS ERROR, NO LOW OR HIGH TIME");
				  }
                #endif
               //              
                // BUS ERROR

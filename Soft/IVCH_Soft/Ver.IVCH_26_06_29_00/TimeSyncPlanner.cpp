...
void TimeSyncPlanner::tick() {
	...
	if (_internet2 && _net2Pending) {
		...
		if (_internet2->readStatus(st)) {
			if (st.lastSyncOk && st.lastNtpUtc != 0 && st.lastNtpUtc != _net2LastSeenNtpUtc) {
				...
				// Apply time to DS3231 (INTERNET2)
				setRtcFromUnixUtc(st.lastNtpUtc);
				pulseSyncLed(SYNC_LED_NET2);

				timeLog("NET2", "OK");
				gTimeOkMsNet2 = now; // NEW:stamp for UI
	...
	if (tryStartNet(now, false)) return; // INTERNET1
	if (tryStartNet2(now, false)) return; // INTERNET2
...
bool TimeSyncPlanner::tryStartGps(uint32_t now, bool immediate)
{
	...
	timeLog("GPS", "OK");
	gTimeOkMsGps = now; // NEW:stamp for UI
	return true;
}
...
bool TimeSyncPlanner::tryStartNet(uint32_t now, bool immediate)
{
	...
	// advance schedule early to avoid hammering
	...
	timeLog("NET1", "OK");
	gTimeOkMsNet1 = now; // NEW:stamp for UI
...
bool TimeSyncPlanner::tryStartGsm(uint32_t now, bool immediate)
{
	...
	// NOTE:TIME GSM OK/FAIL must be logged where SIM time is parsed/applied (in .ino)
	return true;
}
...
//Не забудьте 2 короткие вставки вне этого файла
//A) В.ino(глобально, один раз) :
//	cpp Копировать
//	volatile uint32_t gTimeOkMsGsm = 0;
//B) В вашем блоке после SdLogLine("TIME GSM OK"); :
//cpp Копировать
//if (ext) gTimeOkMsGsm = millis();
//И одно маленькое изменение в menu.cpp
//Чтобы HOME перестал всегда показывать[X], нужно чтобы menu.cpp использовал эти метки gTimeOkMs...
//(как мы обсуждали).Если хотите — пришлите ваш текущий menu.cpp(последнюю версию) целиком, и я верну его полностью уже с заменой chGpsOk / chIp1Ok / chIp2Ok / chGsmOk на метки gTimeOkMs....
//

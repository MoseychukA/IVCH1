...
bool gSdOk = false;
File gLog; // текущий дневной лог
bool gLogTimeOnly = false;
...
bool SetupSD()
{
	...
	// ВАЖНО:ivch.log НЕ открываем и не используем.
	// Дневной лог откроется автоматически при первом SdLogLine().
	return true;
}


volatile uint32_t gTimeOkMsGsm = 0;
extern void SdLogLine(const char* s);
extern bool gSdOk;
extern File gLog;
extern bool gLogTimeOnly; // режим логирования
extern RealtimeClock rtc; // ваш RTC

static constexpr uint16_t LOG_KEEP_DAYS = 30;
static constexpr const char* kLogNoRtcName = "IVNORRTC.LOG"; // 8.3
...
static bool parseDailyLogName(const char* nm, int& y, int& m, int& d)
{
	...
	if (!(nm[8] == '.' && nm[9] == 'L' && nm[10] == 'O' && nm[11] == 'G')) return false;
...
static char gLogCurName[13] = { 0 };
static uint32_t gCleanupKeyYmd = 0;

static void SdLogCleanupOldInternal(const RTCTime& nowT, const char* keepName)
{
	if (!gSdOk) return;
	if (!rtcDateValid(nowT)) return;

	const int32_t nowDays = daysFromCivil((int)nowT.year, (int)nowT.month, (int)nowT.dayOfMonth);
	...
	while (true) {
		...
		// never delete current open file
		if (keepName && strcmp(keepName, nm) == 0) continue;

		// delete no-rtc log when RTC is valid
		if (strcmp(nm, kLogNoRtcName) == 0) {
			SD.remove(nm);
			continue;
		}
		...
		const int32_t ageDays = nowDays - fileDays;

		if (ageDays > (int32_t)LOG_KEEP_DAYS) {
			SD.remove(nm);
		}
	}
...
// Открыть лог при необходимости (и переоткрыть при смене суток)
bool SdLogOpenIfNeeded()
{
	...
	char wantName[13];
	if (rtcDateValid(t)) buildDailyLogName(wantName, t);
	else {
		strncpy(wantName, kLogNoRtcName, sizeof(wantName));
		wantName[sizeof(wantName) - 1] = 0;
	}

	// already open same file
	if (gLog && gLogCurName[0] && (strcmp(gLogCurName, wantName) == 0)) return true;
	...
	gLog = SD.open(wantName, FILE_WRITE);
	if (!gLog) {
		gLogCurName[0] = 0;
		return false;
	}

	strncpy(gLogCurName, wantName, sizeof(gLogCurName) - 1);
	gLogCurName[sizeof(gLogCurName) - 1] = 0;

	// cleanup once per day (only when RTC valid)
	if (rtcDateValid(t)) {
		uint32_t key = (uint32_t)t.year * 10000UL + (uint32_t)t.month * 100UL + (uint32_t)t.dayOfMonth;
		if (gCleanupKeyYmd != key) {
			gCleanupKeyYmd = key;
			SdLogCleanupOldInternal(t, gLogCurName);
		}
	}
...
// Пишем строку с временем (timestamp + msg)
// Пишем строку с временем (timestamp + msg)
// Режимы:
// - gLogTimeOnly=false:пишем всё
// - gLogTimeOnly=true :пишем только "TIME ...",но НЕ пишем "TIME ... SKIP ..."
void SdLogLine(const char* msg)
{
	...
	if (gLogTimeOnly) {
		// only time-related lines
		if (strncmp(msg, "TIME ", 5) != 0) return;
		// exclude SKIP lines even though they start with TIME
		if (strstr(msg, " SKIP ") != nullptr) return;
	}
...
//gLogTimeOnly = true; // только TIME REQ/RECV/OK/FAIL (без SKIP)
//gLogTimeOnly = false; // полный лог
...
static const char* checkpointName(uint16_t id)
{
	...
	case CHK_BOOT:return "ЗАГРУЗКА";
	case CHK_LOOP_BEGIN:return "НАЧАЛО ЦИКЛА";
...
static void watchdogInit()
{
	...
	if (!IWatchdog.isEnabled()) {
		...
		SdLogLine("WDT:НЕ УДАЛОСЬ ВКЛЮЧИТЬ");
	}
	else 
	{
		SdLogLine("WDT:ВКЛЮЧЕНО");
	}
}
...
void loop()
{
	...
	if(sim.hasNewTime())
	{
		...
		// NEW:это результат внешнего запроса (planner/UI)?
		// Если false — это внутренний периодический опрос SIM800 и логи OK/FAIL писать нельзя
		// (иначе будут OK/FAIL без REQ).
		const bool ext = sim.consumeExternalTimeResultFlag();

		// Сначала проверяем валидность CCLK (год/формат),
		// чтобы различать FAIL bad_cclk и FAIL parse
		if (!sim.lastTimeValid(2020))
		{
			if (ext) SdLogLine("TIME GSM FAIL bad_cclk");
			watchdogKick();
		}
		else
		{
			...
			if (SIM800TimeAsync::parseCCLK(raw, y, mo, d, h, mi, s, tzq_net))
			{
				...
				// STRICT:OK логируем только если это был внешний запрос (и только после применения времени)
				if (ext) SdLogLine("TIME GSM OK");
				if (ext) gTimeOkMsGsm = millis();
			...
			{
				if (ext) SdLogLine("TIME GSM FAIL parse");
			}

...
// RSTn W5500 у вас подключен к PB1
...
struct NtpLanService_Generic::Impl
{
	...
	// если сохранили настройки при отключенном линке — применим когда линк появится
	bool pendingLanApply = false;

	Impl(RealtimeClock& r, AT24C128Settings::Config& c)
		:rtc(r), cfg(c)
	{}

	IPAddress localIP() const { return Ethernet.localIP(); }

	// Аппаратный reset W5500 (RSTn активный LOW)
	void w5500HardReset()
	{
...
#else
		// если вдруг соберется не на STM32 — ничего не делаем
#endif
	}

	// Надёжная проверка линка (после того как чип инициализирован)
	bool linkUp() const
	{
		W5100Linkstatus st = W5100.getLinkStatus();
		return (st == LINK_ON);
	}
	...
	static uint32_t unixFromRtcLocal(const RTCTime& t, int8_t tzTargetHours)
	{
		...
		int64_t local = (int64_t)days * 86400LL
	...
	void ethPrepareBus()
	{
		// направляем Ethernet_Generic на нужный SPI
		extern SPIClass* pCUR_SPI;
		pCUR_SPI = &SPI_New;

		// CS всегда PA4 (USE_THIS_SS_PIN)
		pinMode(USE_THIS_SS_PIN, OUTPUT);
		digitalWrite(USE_THIS_SS_PIN, HIGH);

		// инициализируем SPI и сообщаем Ethernet чипу правильный CS
		SPI_New.begin();
		Ethernet.init(USE_THIS_SS_PIN);
	}

	void ethPrintCfg(const __FlashStringHelper* tag)
	{
		SerialDebug.print(tag);
		SerialDebug.print(F(" localIP=")); SerialDebug.println(Ethernet.localIP());
		SerialDebug.print(tag);
		SerialDebug.print(F(" gw=")); SerialDebug.println(Ethernet.gatewayIP());
		SerialDebug.print(tag);
		SerialDebug.print(F(" mask=")); SerialDebug.println(Ethernet.subnetMask());
		SerialDebug.print(tag);
		SerialDebug.print(F(" dns=")); SerialDebug.println(Ethernet.dnsServerIP());
		SerialDebug.print(tag);
		SerialDebug.print(F(" link=")); SerialDebug.println(Ethernet.linkReport());
	}
	...
	bool begin(const NetConfig& nc)
	{
		...
		// Важно для стабильного DHCP/линка:жёсткий reset перед первой инициализацией
		w5500HardReset();

		ethPrepareBus();

		const uint16_t idx = net.macIndex % NUMBER_OF_MAC;

		if (net.useDhcp)
		{
			// Не проверяем линк до первого Ethernet.begin():linkReport может быть NOT SUPPORTED.
			SerialDebug.println(F("[ETH] DHCP begin..."));
			ethPrepareBus();
			int ok = Ethernet.begin(mac[idx], 3000, 1000);
			ethPrintCfg(F("[ETH][dhcp]"));

			if (!ok)
			{
				SerialDebug.println(F("[ETH] DHCP failed -> reset + retry DHCP"));

				// Часто помогает,если PHY/чип “нечисто” стартанул
				w5500HardReset();
				ethPrepareBus();
				ok = Ethernet.begin(mac[idx], 3000, 1000);
				ethPrintCfg(F("[ETH][dhcp-retry]"));
			}
	...
	void handleSyncClient()
	{
		...
		rtc.setTime(
			tLocal.second, tLocal.minute, tLocal.hour,
	...
	void tick()
	{
		// Если линка нет — не делаем NTP и не трогаем maintain()
		if (!linkUp())
			return;
	...
	void applyLanConfig(bool dhcp, IPAddress ip, IPAddress dns, IPAddress gw, IPAddress mask)
	{
		...
		// Если линка нет — отложим применение (чтобы не блокировать и не плодить ошибки)
		if (!linkUp())
		{
		...
		if (net.useDhcp)
		{
			SerialDebug.println(F("[ETH] applyLanConfig:DHCP begin..."));
			...
			if (!ok)
			{
				SerialDebug.println(F("[ETH] applyLanConfig:DHCP failed -> fallback to static"));
				ethPrepareBus();
				Ethernet.begin(mac[idx], net.ip, net.dns, net.gw, net.mask);
				ethPrintCfg(F("[ETH][static]"));
				net.useDhcp = false;

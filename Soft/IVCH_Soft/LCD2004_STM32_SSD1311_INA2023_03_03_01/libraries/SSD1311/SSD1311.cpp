...
SSD1311::SSD1311(){
	...
	display_lines = 4;// @TODO control this, write function to change registers N and NW
	display_shift = true;
...
void SSD1311::print(const char* String) {
	unsigned char i = 0;
	char ch;
	char ch1;
	while (String[i]) 
	{
		...
		if ((ch1 > 191) && (ch1 < 224))        // Ограничить прием символа. Только большие русские буквы
		{
			ch1 -= 64;                         // Переместить код символа в кодировку дисплея.  Только большие русские буквы
			ch = ch1;
		}
		else if ((ch1 > 223) && (ch1 < 256))  // Ограничить прием символа в кодировке Win1251. Только малые русские буквы
		{
			ch1 -= 96;                          // Переместить код символа в кодировку дисплея.  Только большие русские буквы
			ch = ch1;
		}
		else if ((ch1 > 20) && (ch1 < 128))    // Ограничить прием символа в кодировке Win1251. Только латинские буквы
		{
			ch = ch1;
		}
		else
		{ //Латиница
			ch = 46;                           // Назначить всем символам вне диапазона кодовых страниц символ "точки"
		}
...
void SSD1311::printRus(const char* String, uint8_t col, uint8_t row) 
{
	setCursor(col, row);
	unsigned char i = 0;
	char ch;
	...
	{
		ch = String[i];
		if ((ch > 20) && (ch < 128))    // Ограничить прием символа в кодировке Win1251. Только латинские буквы
		{
			sendData(ch);
		}
		else if ((ch > 143) && (ch < 176))       // Ограничить прием символа в кодировке Win1251. Только большие русские буквы
		{
			ch -= 16;                        // Переместить код символа в кодировку дисплея.  Только большие русские буквы
			sendData(ch);
		}
		i++;
	}
...
void SSD1311::startscrollright(uint8_t start, uint8_t stop) {
	
	/*static const uint8_t PROGMEM scrollList1a[] = { SSD1311_RIGHT_HORIZONTAL_SCROLL, 0x00 };
	ssd1306_commandList(scrollList1a, sizeof(scrollList1a));*/

	sendCommand(SSD1311_RIGHT_HORIZONTAL_SCROLL);
	sendCommand(0x00);
	...
	sendCommand(0x00);
	sendCommand(stop);
	//static const uint8_t PROGMEM scrollList1b[] = { 0x00, 0xFF, SSD1311_ACTIVATE_SCROLL };
	//ssd1306_commandList(scrollList1b, sizeof(scrollList1b));

	sendCommand(0x00);
	sendCommand(0xFF);
	sendCommand(SSD1311_ACTIVATE_SCROLL);
}
...
void SSD1311::startscrollleft(uint8_t start, uint8_t stop) {

	/*static const uint8_t PROGMEM scrollList2a[] = { SSD1311_LEFT_HORIZONTAL_SCROLL, 0x00 };
	ssd1306_commandList(scrollList2a, sizeof(scrollList2a));*/

	sendCommand(SSD1311_LEFT_HORIZONTAL_SCROLL);
	sendCommand(0x00);
	...
	//static const uint8_t PROGMEM scrollList2b[] = { 0x00, 0xFF, SSD1311_ACTIVATE_SCROLL };
...
void SSD1311::startscrolldiagright(uint8_t start, uint8_t stop) {
	/*
	static const uint8_t PROGMEM scrollList3a[] = { SSD1311_SET_VERTICAL_SCROLL_AREA, 0x00 };
	ssd1306_commandList(scrollList3a, sizeof(scrollList3a));
	sendCommand(HEIGHT);
	static const uint8_t PROGMEM scrollList3b[] = { SSD1311_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL, 0x00 };
	ssd1306_commandList(scrollList3b, sizeof(scrollList3b));
	sendCommand(start);
	sendCommand(0x00);
	sendCommand(stop);
	static const uint8_t PROGMEM scrollList3c[] = { 0x01, SSD1311_ACTIVATE_SCROLL };
...
void SSD1311::startscrolldiagleft(uint8_t start, uint8_t stop) {

	/*static const uint8_t PROGMEM scrollList4a[] = { SSD1311_SET_VERTICAL_SCROLL_AREA, 0x00 };
	ssd1306_commandList(scrollList4a, sizeof(scrollList4a));
	sendCommand(HEIGHT);
	static const uint8_t PROGMEM scrollList4b[] = {SSD1311_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL, 0x00 };
	ssd1306_commandList(scrollList4b, sizeof(scrollList4b));
	sendCommand(start);
	sendCommand(0x00);
	sendCommand(stop);
	static const uint8_t PROGMEM scrollList4c[] = { 0x01, SSD1311_ACTIVATE_SCROLL };
...
/*!
	...
	@return None (void).
*/
void SSD1311::stopscroll(void) {

	sendCommand(SSD1311_DEACTIVATE_SCROLL);
}
...
void SSD1311::setCursor(uint8_t col, uint8_t row) {
	...
	uint8_t row_offsets[] = { 0x80, 0xA0, 0xC0, 0xE0 };

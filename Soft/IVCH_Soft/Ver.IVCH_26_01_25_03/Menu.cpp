...
// RAM limit:use small reusable sprites instead of full-screen (320x170 would be too big for 96KB).
// We need two widths because dynamic text starts at different X:
// - IP1/IP2/DATE/SIG:x=80,max width=240 (fits exactly to 320)
// - NET:x=100,max width=220 (fits exactly to 320)
static TFT_eSprite* gHomeSpr240 = nullptr; // 240x20
static TFT_eSprite* gHomeSpr220 = nullptr; // 220x20
static TFT_eSPI* gHomeSprOwner = nullptr;

static void homeSpritesFree()
{
	if (gHomeSpr240) { gHomeSpr240->deleteSprite(); delete gHomeSpr240; gHomeSpr240 = nullptr; }
	if (gHomeSpr220) { gHomeSpr220->deleteSprite(); delete gHomeSpr220; gHomeSpr220 = nullptr; }
	if (gHomeSpr240) { gHomeSpr240->unloadFont(); gHomeSpr240->deleteSprite(); delete gHomeSpr240; gHomeSpr240 = nullptr; }
	if (gHomeSpr220) { gHomeSpr220->unloadFont(); gHomeSpr220->deleteSprite(); delete gHomeSpr220; gHomeSpr220 = nullptr; }
...
static void homeSpritesEnsure(TFT_eSPI* tft)
{
	...
	if (!gHomeSpr240) {
		...
		if (gHomeSpr240) {
			...
			if (gHomeSpr240->createSprite(240, 20) == nullptr) {
				delete gHomeSpr240; gHomeSpr240 = nullptr;
			}
			else {
	...
	if (!gHomeSpr220) {
		gHomeSpr220 = new (std::nothrow) TFT_eSprite(tft);
		if (gHomeSpr220) {
			gHomeSpr220->setColorDepth(16);
			if (gHomeSpr220->createSprite(220, 20) == nullptr) {
				delete gHomeSpr220; gHomeSpr220 = nullptr;
			}
			else {
				gHomeSpr240->loadFont(AA_FONT_TIME18);
				gHomeSpr220->setTextColor(TFT_WHITE, TFT_BLACK);
			}
		}
	}
}
...
void Menu::drawHomeDynamic()
{
	...
	if (_lastNet != gNetRegistered) {
		_lastNet = gNetRegistered;
		homePushText(gHomeSpr220, 80, Y_NET, gNetRegistered ? TFT_GREEN : TFT_RED,
	...
	if (_lastBars != gSignalBars) {
		...
		homePushText(gHomeSpr240, 80, Y_SIG, TFT_WHITE, buf);
	}
}

...
// -------------------- Channel OK predicates (time-data freshness) --------------------
...
{
	// REAL time receive for INTERNET1:
	// NtpLanService_Generic updates lastSuccessMs only on successful time set.
	if (syncData.netEnable == 0) return false;
	const uint32_t lastOk = ntpLan.lastSuccessMs();
	return isFreshByPeriod(lastOk, periodIdxToMs(syncData.netPeriodIdx), now);
}

static inline bool chIp2Ok(uint32_t now)
{
	// REAL time receive for INTERNET2:
	// derived from change of lastNtpUtc while lastSyncOk==true.
	if (syncData.net2Enable == 0) return false;
	return isFreshByPeriod(gNet2TimeRxMs, periodIdxToMs(syncData.net2PeriodIdx), now);
}

static inline bool chGsmOk(uint32_t now)
{
	// still a proxy (no real time receive timestamp in provided sources)
	return (syncData.gsmEnable != 0) &&
		isFreshByPeriod(gGsmRxMs, periodIdxToMs(syncData.gsmPeriodIdx), now);
}
...
private:
	...
	void drawStatusLine()
	{
		...
		// clear only left part,keep marker zone intact
...
void Menu::update()
{
	...
	// Poll real time-receive status for INTERNET2
	pollInternet2TimeStatus(now);

	// Update GSM RX proxy + LEDs always
	...
	if (state == MENU_PAGE_SELECT) {
		// update only marker,avoid full redraw
...
void Menu::drawHomeDynamic()
{
	...
	const bool ip1Ok = chIp1Ok(now); // REAL (ntpLan.lastSuccessMs)
	const bool ip2Ok = chIp2Ok(now); // REAL (derived from Internet2 status)

...
private:
	...
	uint8_t paramCount() const {
		...
		if (_mode == MODE_NET) return 5; // enable,server,period,TZ target,TZ NTP
	...
	void buildLine(uint8_t i, char* out, size_t n) const {
		...
		if (_mode == MODE_GPS) {
			if (i == 0) snprintf(out, n, "1) ВКЛЮЧИТЬ: %s", _tmp.gpsEnable ? "ДА" : "НЕТ");
		...
		if (_mode == MODE_NET) {
			if (i == 0) snprintf(out, n, "1) ВКЛЮЧИТЬ: %s", _tmp.netEnable ? "ДА" : "НЕТ");
		...
		// MODE_GSM (5 пунктов)
		if (i == 0) snprintf(out, n, "1) ВКЛЮЧИТЬ: %s", _tmp.gsmEnable ? "ДА" : "НЕТ");
		else if (i == 1) snprintf(out, n, "2) ОПЕРАТОР: %s", kGsmProviders[_tmp.gsmProviderIdx % 5]);
	...
	{
		// GPS:0 enable,1 period,2 tzTarget,3 tzNtp
		if (_mode == MODE_GPS) {
		...
		// NET:0 enable,1 server,2 period,3 tzTarget,4 tzNtp
		if (_mode == MODE_NET) {
		...
		// GSM (как было):0 enable,1 operator,2 period,3 tzTarget,4 tzNtp
	...
	void commitSave() 
	{
		...
		// Любой режим (GPS/NET/GSM):если TZ меняли — сохраняем в cfg + EEPROM cfg

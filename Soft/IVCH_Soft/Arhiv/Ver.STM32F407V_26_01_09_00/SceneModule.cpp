...
void SceneModule::AddSceneToList(uint16_t sceneNumber)
{
  ...
  {
    #ifdef SCENE_DEBUG
	  if (debug.SCENE_DEBUG_K == HIGH)
	  {
		  Serial4.println(F("Scene not found!"));
	  }
    #endif
    return;
  }
  ...
  if (debug.SCENE_DEBUG_K == HIGH)
  {
	  Serial4.println(F("Add scene to list and execute first step..."));
  }
  #endif
...
void SceneModule::ExecuteStep(uint16_t sceneNumber)
{
  ...
  if(!sceneFound)
  {
	  ...
	  if (debug.SCENE_DEBUG_K == HIGH)
	  {
		  Serial4.println(F("Scene not found in list!"));
	  }
    #endif
    return;
  }
  ...
  if (debug.SCENE_DEBUG_K == HIGH)
  {
	  Serial4.print(F("CHECK STEP EXISTS - "));
	  Serial4.println(dirName);
  }
  #endif
  ...
  if(!SDFat.exists(dirP))
  {
	  ...
	  if (debug.SCENE_DEBUG_K == HIGH)
	  {
		  Serial4.println(F("Step NOT EXISTS!"));
	  }
    #endif
    ...
    if(list[sceneIndex].stepNumber == 0 || !list[sceneIndex].continuousMode)
    {
		...
		if (debug.SCENE_DEBUG_K == HIGH)
		{
			Serial4.println(F("SCENE DONE, remove from list!"));
		}
      #endif
      ExecuteSceneFile(sceneNumber, stopFile);
      RemoveSceneFromList(sceneNumber);
    ...
    else
    {
		...
		if (debug.SCENE_DEBUG_K == HIGH)
		{
			Serial4.println(F("Wrap to scene beginning!"));
		}
      #endif
  ...
  if(f.open(dirP,FILE_READ))
  {
	...
	if (debug.SCENE_DEBUG_K == HIGH)
	{
		Serial4.print(F("Step duration, s = "));
		Serial4.println(duration);
	}
    #endif
  ...
  else
  {
	  ...
	  if (debug.SCENE_DEBUG_K == HIGH)
	  {
		  Serial4.println(F("Unable to open step file, skip it!"));
	  }
    #endif
...
void SceneModule::ExecuteSceneFile(uint16_t sceneNumber, SceneFile whichFile)
{
  ...
  if(SDFat.exists(dirP))
  {
	  ...
	  if (debug.SCENE_DEBUG_K == HIGH)
	  {
		  Serial4.print(F("Execute scene file - "));
		  Serial4.println(dirName);
	  }
    #endif

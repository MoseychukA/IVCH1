...
void WateringChannel::ChangeHardwareState()
{
    ...
    if(wbnd.ChannelsLinkType != linkUnbinded)
    {
	  ...
	  if (relayPin != UNBINDED_PIN)
	  {
       ...
       if(wbnd.ChannelsLinkType == linkPortExpander)
       {
              ...
              WORK_STATUS.PCF8575_PinWrite(wbnd.ChannelsMCPAddress,relayPin,state);              
            #endif
       }
...
WateringChannel::WateringChannel()
{
  ...
  flags.activeWindowStartMinute = 0xFFFF;
  flags.activeWindowIndex = -1;
  _timer = 0;
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void WateringChannel::Setup(uint8_t index)
{
    flags.index = index;
    ...
    if(wbnd.ChannelsLinkType != linkUnbinded)
    {
      ...
      if(relayPin != UNBINDED_PIN)
      {
        ...
        if(wbnd.ChannelsLinkType == linkPortExpander)
        {
          ...
          #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
            WORK_STATUS.PCF8575_PinMode(wbnd.ChannelsMCPAddress,relayPin,OUTPUT);
          #endif
        }
      } // if(relayPin != UNBINDED_PIN)
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ (v2.1):
// - если окно выходит за полночь,обрезаем конец до 24:00 (1440)
// - если окна начинают пересекаться (интервал < длительности) — прекращаем планирование
static bool GetActiveWindow(
    uint16_t nowMin,
    uint16_t startMin,
    uint16_t durationMin,
    uint16_t count,
    uint32_t intervalMin,
    uint16_t& winStartMin,
    uint16_t& winEndMin,
    int8_t& winIndex
)
{
    const uint16_t DAY_MINUTES = 1440;
    ...
    if (startMin >= DAY_MINUTES) return false;

    uint16_t realCount = (count == 0) ? 1 : count;
    if (realCount > 48) realCount = 48;

    if (realCount > 1 && intervalMin == 0) realCount = 1;

    uint32_t prevEnd = 0;

    for (uint16_t i = 0; i < realCount; i++)
    {
        uint32_t s = (uint32_t)startMin + (uint32_t)i * intervalMin;
        if (s >= DAY_MINUTES) break;

        uint32_t e = s + (uint32_t)durationMin;
        if (e > DAY_MINUTES) e = DAY_MINUTES; // ОБРЕЗКА ПО ПОЛУНОЧИ

        if (e <= s) break; // на всякий случай

        // не допускаем пересечения окон
        if (i > 0 && s < prevEnd)
        {
            break;
        }
        ...
        {
            winStartMin = (uint16_t)s;
            winEndMin = (uint16_t)e;
            winIndex = (int8_t)i;
            return true;
        }
    }
...
static bool GetActiveWindowSeconds(
    uint32_t nowSec,
    uint16_t startMin,
...
)
{
    ...
    if (startMin >= 1440) return false;

    uint16_t realCount = (count == 0) ? 1 : count;
    if (realCount > 48) realCount = 48;
    if (realCount > 1 && intervalSec == 0) realCount = 1;

    for (uint16_t i = 0; i < realCount; i++)
    {
        uint32_t s = (uint32_t)startMin * 60ul + (uint32_t)i * intervalSec;
        if (s >= DAY_SECONDS) break;

        uint32_t e = s + durationSec;
        if (e > DAY_SECONDS) e = DAY_SECONDS; // обрезка по полуночи

        if (nowSec >= s && nowSec < e)
        {
            winStartMin = (uint16_t)(s / 60ul);
            winEndMin = (uint16_t)(e / 60ul);
            winIndex = (int8_t)i;
            return true;
        }
    }
 ...
 // проверяем - есть ли задержка перед началом работы?
  ...
  if(flags.powerTimerDelay > 0) // ещё есть задержка перед записью в выход, для устаканивания питания
  {
    ...
    if(currentWateringOption == wateringOFF) // автоматическое управление каналами выключено, не надо обновлять состояние канала
    {
     ...
     uint32_t timeToWatering = currentWateringOption == wateringWeekDays ? settings->GetWateringTime() : settings->GetChannelWateringTime(flags.index);

     // Флаг формата длительности: 0=минуты, 1=секунды
     uint8_t intervalSecond = settings->GetChannelWateringSecond((currentWateringOption == wateringWeekDays) ? 0 : flags.index);

     // на всякий случай нормализуем
     if (intervalSecond > 1) intervalSecond = 0;
     // длительность для таймера (мс)
     uint32_t wateringDurationMs = intervalSecond ? (timeToWatering * 1000ul) : (timeToWatering * 60000ul);
     // длительность для окон (минуты, RTC минутный)
     uint16_t durationForWindowMin = intervalSecond ? (uint16_t)max(1ul, (timeToWatering + 59ul) / 60ul) : (uint16_t)timeToWatering;

     //Индекс датчика влажности грунта
     ...
     // Флаг остановки полива по показаниям датчика влажности грунта
     ...
     // Флаг старта полива по показаниям датчика влажности грунта
     ...
     // количество поливов в сутки
     uint16_t startCount = currentWateringOption == wateringWeekDays ? settings->GetWateringCount() : settings->GetChannelWateringCount(flags.index);
     // Интервал между поливами в сутки
     uint16_t startIntervalHours = currentWateringOption == wateringWeekDays ? settings->GetWateringInterval() : settings->GetChannelWateringInterval(flags.index);

     // интервал в минутах,БЕЗ переполнения
     uint32_t intervalMinutes = (uint32_t)startIntervalHours * 60ul;
     ...
     uint32_t currentTimeInSeconds = (uint32_t)currentTime.hour * 3600ul
         + (uint32_t)currentTime.minute * 60ul
         + (uint32_t)currentTime.second;

     bool weekDayAllowed = bitRead(weekDays, currentTime.dayOfWeek - 1);

     uint16_t winStart = 0xFFFF;
     uint16_t winEnd = 0xFFFF;
     int8_t winIndex = -1;

     // длительность окна в секундах (по флагу)
     uint32_t durationSec = intervalSecond
         ? (uint32_t)timeToWatering            // timeToWatering в секундах
         : (uint32_t)timeToWatering * 60ul;    // timeToWatering в минутах

       // интервал между поливами в секундах (из часов)
     uint32_t intervalSec = (uint32_t)startIntervalHours * 3600ul;

     bool inWindow = false;
     if (weekDayAllowed)
     {
         inWindow = GetActiveWindowSeconds(
             currentTimeInSeconds,
             startWateringTime, // старт всё равно в минутах от 00:00
             durationSec,
             startCount,
             intervalSec,
             winStart,
     ...
     bool canWork = weekDayAllowed && inWindow;

     // Если началось новое окно (следующий полив в текущие сутки) — сбрасываем таймер
     if (canWork)
     {
         if (flags.activeWindowStartMinute != winStart || flags.activeWindowIndex != winIndex)
         {
             flags.activeWindowStartMinute = winStart;
             flags.activeWindowIndex = winIndex;

             flags.wateringTimer = 0;

             // чтобы сохранение по минутам продолжало работать корректно
    ...
    if(!canWork)
     { 
        // если выключаемся из активного состояния — один раз сохраним фактическое отработанное время
        if (IsActive())
        {
     ...
     else
     {
      ...
      uint32_t maxTimeToWatering = (wateringDurationMs + dt);

      if(flags.wateringTimer > maxTimeToWatering) // время вышло, надо выключать полив
      {
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void WateringChannel::DoLoadState(uint8_t  addressOffset)
{
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void WateringChannel::DoSaveState(uint8_t addressOffset,uint32_t wateringTimer)
{  
	SysDebug debug = HardwareBinding->GetSysDebug();
#ifdef USE_DS3231_REALTIME_CLOCK
...
void WateringModule::TurnPump1(bool isOn)
{
  ...
  if(wbnd.PumpLinkType != linkUnbinded && wbnd.PumpPin != UNBINDED_PIN)
  {
      ...
      if(wbnd.PumpLinkType ==  linkPortExpander)
      {
        #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
        ...
        #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
          WORK_STATUS.PCF8575_PinWrite(wbnd.PumpMCPAddress,wbnd.PumpPin,state);
        #endif
      }
...
void WateringModule::TurnPump2(bool isOn)
{
  ...
  {  
      ...
      if(wbnd.Pump2LinkType == linkPortExpander)
      {
        #if defined(USE_MCP23S17_EXTENDER) && COUNT_OF_MCP23S17_EXTENDERS > 0
        ...
        #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
          WORK_STATUS.PCF8575_PinWrite(wbnd.Pump2MCPAddress,wbnd.Pump2Pin,state);
        #endif
      }
...
void WateringModule::SetupPumps()
{
    ...
    if(wbnd.PumpLinkType != linkUnbinded && wbnd.PumpPin != UNBINDED_PIN)
    {
      ...
      if(wbnd.PumpLinkType ==  linkPortExpander)
      {
        ...
        #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
          WORK_STATUS.PCF8575_PinMode(wbnd.PumpMCPAddress,wbnd.PumpPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(wbnd.PumpMCPAddress,wbnd.PumpPin,!wbnd.PumpLevel);
        #endif
      }
    } // if(wbnd.PumpLinkType != linkUnbinded)
    ...
    if(wbnd.Pump2LinkType != linkUnbinded && wbnd.Pump2Pin != UNBINDED_PIN)
    {
      ...
      if(wbnd.Pump2LinkType ==  linkPortExpander)
      {
        ...
        #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
          WORK_STATUS.PCF8575_PinMode(wbnd.Pump2MCPAddress,wbnd.Pump2Pin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(wbnd.Pump2MCPAddress,wbnd.Pump2Pin,!wbnd.PumpLevel);
        #endif
      }
    } // if(wbnd.Pump2LinkType != linkUnbinded)
        ...
        if(which == WATER_SETTINGS_COMMAND) // CTSET=WATER|T_SETT|WateringOption|WateringDays|WateringTime|StartTime|TurnOnPump|wateringSensorIndex|wateringStopBorder|switchToAutoAfterMidnight|wateringStartBorder(optional)
        {
          if(argsCount > 5)
          {
             ...
             switchToAutoAfterMidnight = 1;
              ...
              uint8_t wateringCount = 0;
              if (argsCount > 10)
              {
                  wateringCount = (uint8_t)atoi(command.GetArg(10));
              }

              uint8_t wateringInterval = 0;
              if (argsCount > 11)
              {
                  wateringInterval = (uint8_t)atoi(command.GetArg(11));
              }
              ...
              settings->SetWateringCount(wateringCount);
              settings->SetWateringInterval(wateringInterval);
        ...
        if(which == WATER_CHANNEL_SETTINGS) // настройки канала CTSET=WATER|CH_SETT|IDX|WateringDays|WateringTime|StartTime|wateringSensorIndex|wateringStopBorder|wateringStartBorder(optional)
        {
           if(argsCount > 4)
           {
                ...
                if(channelIdx < WATER_RELAYS_COUNT)
                {
                  ...
                  if (argsCount > 8)
                  {
                      wateringCount = (uint8_t)atoi(command.GetArg(8));
                  }

                  uint8_t wateringInterval = 0;
                  if (argsCount > 9)
                  {
                      wateringInterval = (uint8_t)atoi(command.GetArg(9));
                  }
                  ...
                  settings->SetChannelWateringCount(channelIdx, wateringCount);
                  settings->SetChannelWateringInterval(channelIdx, wateringInterval);
           ...
           if(argsCount > 1)
           {
                ...
                if(t == WATER_CHANNEL_SETTINGS)
                {
                  ...
                  if(idx < WATER_RELAYS_COUNT)
                  {
                    ...
                    << (settings->GetChannelWateringStartBorder(idx)) << PARAM_DELIMITER
                    << (settings->GetChannelWateringCount(idx)) << PARAM_DELIMITER
                    << (settings->GetChannelWateringInterval(idx));
                  }
                  else
                  {
                    // плохой индекс

...
void SMSModule::IncomingCUSD(const String& cusd)
{
    ...
    // по CUSD мы запрашиваем только баланс, и просто пересылаем его хозяину и его помощнику
    if (MemRead(SMS_NUMBER_EEPROM_ADDR1) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR1))
    {
        String phoneNum = MainController->GetSettings()->GetSmsPhoneNumber(1);
        SIM800.sendSMS(phoneNum, cusd, true);
    }
    if (MemRead(SMS_NUMBER_EEPROM_ADDR2) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR2))
    {
        String phoneNum = MainController->GetSettings()->GetSmsPhoneNumber(2);
        SIM800.sendSMS(phoneNum, cusd, true);
    }
}
//--------------------------------------------------------------------------------------------------------------------------------
...
{
  #ifdef GSM_DEBUG_MODE
      Serial4.print(F("SIM800: INCOMING SMS RECEIVED: "));
  ...
  if(message.startsWith(SMS_RESET_COMMAND))
  {
    ...
    if(idx != -1)
    {
      ...
      // посылаем баланс хозяину и его помощнику
      RequestBalance();
...
void SMSModule::SendStatToTank(const String& phoneNum)
{
	...
	else
	{
		...
		{
			#ifdef GSM_DEBUG_MODE
			Serial4.println(F("СБОЙ НАПОЛНЕНИЯ"));
			#endif
			sms = "СБОЙ НАПОЛНЕНИЯ";
		}
		...
		{
			#ifdef GSM_DEBUG_MODE
				Serial4.println(F("СБОЙ ДАТЧИКА #1"));
			#endif
			sms = "СБОЙ ДАТЧИКА #1";
		}
...
{ 
    ...
    if (SIM800.ready())
    {
        if (!workStartedSmsSent)
        {
            ...
            {
                if (MemRead(SMS_NUMBER_EEPROM_ADDR1) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR1) && MemRead(MODE_NUMBER_EEPROM_ADDR1 + 1))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR1, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
                }
                if (MemRead(SMS_NUMBER_EEPROM_ADDR2) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR2) && MemRead(MODE_NUMBER_EEPROM_ADDR2 + 1))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR2, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
                }
                if (MemRead(SMS_NUMBER_EEPROM_ADDR3) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR3) && MemRead(MODE_NUMBER_EEPROM_ADDR3 + 1))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR3, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
                }
                if (MemRead(SMS_NUMBER_EEPROM_ADDR4) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR4) && MemRead(MODE_NUMBER_EEPROM_ADDR4 + 1))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR4, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
                }
            }
    ...
    if (millis() - alarmTimer > 10000) // незачем чаще чем раз в 10 секунд проверять тревоги
    {
        ...
        if (SIM800.ready())
        {
            ...
            if (alD->HasSMSAlarm())
            {
                ...
                if (MemRead(SMS_NUMBER_EEPROM_ADDR3) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR3))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR3, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
                }
                if (MemRead(SMS_NUMBER_EEPROM_ADDR4) == 43 && MemRead(MODE_NUMBER_EEPROM_ADDR4))
                {
                    tel_start = RulesDispatcher->readString(SMS_NUMBER_EEPROM_ADDR4, 15);
                    SIM800.sendSMS(tel_start, WORK_STARTED_SMS_TEXT, false);
                    tel_start = "";
  ...
  if(command.GetType() == ctGET) //получить статистику
  {
    ...
    else
    {
      String t = command.GetArg(0);
          ...
          {
              if (MainController->GetSettings()->GetNumberIncomingSMS() != "")
              {
                  SendStatToCaller(Settings->GetNumberIncomingSMS()); // посылаем статистику на указанный номер телефона
              }
              MainController->GetSettings()->SetNumberIncomingSMS("");
              PublishSingleton.Flags.Status = true;

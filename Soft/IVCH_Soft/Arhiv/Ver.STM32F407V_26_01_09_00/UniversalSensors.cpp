...
void UniRS485Gate::enableSend()
{
  ...
  if(bnd.LinkType != linkUnbinded && bnd.DEPin != UNBINDED_PIN)
  {
    ...
    if(bnd.LinkType == linkPortExpander)
    {
        ...
        WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.DEPin, HIGH);
      #endif
    }
...
void UniRS485Gate::enableReceive()
{
  ...
  if(bnd.LinkType != linkUnbinded && bnd.DEPin != UNBINDED_PIN)
  {
    ...
    if(bnd.LinkType == linkPortExpander)
    {
      ...
      #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
        WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.DEPin, LOW);
      #endif
    }
  } // if(bnd.LinkType != linkUnbinded  
...
void UniRS485Gate::Setup()
{
  ...
  if(bnd.LinkType != linkUnbinded && bnd.DEPin != UNBINDED_PIN)
  {
    ...
    if(bnd.LinkType == linkPortExpander)
    {
      ...
      #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
        WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress, bnd.DEPin, OUTPUT);
      #endif
    }
  } // if(bnd.LinkType != linkUnbinded  
...
void UniRS485Gate::sendFillTankCommand(bool on)
{
	...
	if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	{
		 Serial4.print(F("RS-485: Send WATER TANK command packet with valve command: "));
		 Serial4.println(on);
	}
...
void UniRS485Gate::sendWaterTankSettingsPacket()
{
	...
	if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	{
		 Serial4.println(F("RS-485: Send WATER TANK settings packet..."));
	}
         #endif // WATER_TANK_MODULE_DEBUG  
...
{
 //   Serial4.println("collectSensorsForRemoteDisplay()");
 ...
 //  Serial4.println(sensorsCount);
  ...
  for(uint8_t i=0;i<sensorsCount;i++)
  {
   ...
   //  Serial4.println(REMOTE_DISPLAY_SENSOR_SETTINGS[i].moduleName);
  ...
  //  Serial4.println("collectSensorsForRemoteDisplay() DONE.");
...
void UniRS485Gate::Update()
{
  ...
  WindSensorBinding bndWind = HardwareBinding->GetWindSensorBinding();   // Датчик скорости ветра
  RainSensorBinding bndRain = HardwareBinding->GetRainSensorBinding();   // Датчик дождя
  ...
  if(remoteDisplayTimer > REMOTE_DISPLAY_UPDATE_INTERVAL)
  {
   ...
   //  Serial4.println("Collect sensors for remote display...");
    ...
    {
       // Serial4.println("no data for display, collect it...");
    ...
    // Serial4.println(data->sensorsInPacket);
...
///     enableReceive();    
   ...
   //     Serial4.println("packet was sent.");
...
void WaterTankClient::Update(UniRawScratchpad* scratchpad, bool isModuleOnline, UniScratchpadSource receivedThrough)
{
    ...
    if(ssRadio == receivedThrough)
    {
	  ...
	  if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	  {
		   Serial4.println(F("RECEIVED WATER TANK DATA VIA RADIO:"));
		   Serial4.print(F("\tValve state: "));  Serial4.println(waterTankData->valveState);
		   Serial4.print(F("\tFill status, %: "));  Serial4.println(waterTankData->fillStatus);
		   Serial4.print(F("\tErrors status - "));  Serial4.println(waterTankData->errorFlag); //добавил еще вывод флага ошибки в дебаг-сообщение - КМВ
		   Serial4.print(F("\tErrors type - "));  Serial4.println(waterTankData->errorType); //добавил еще вывод флага ошибки в дебаг-сообщение - КМВ
	  }
     #endif
...
void WindRainClient::Update(UniRawScratchpad* scratchpad, bool isModuleOnline, UniScratchpadSource receivedThrough)
{
    ...
    if(ssRadio == receivedThrough)
    {
	    ...
	    if (debug.NRF_DEBUG_K == HIGH || debug.LORA_DEBUG_K == HIGH)
	    {
...
void UniNRFGate::Setup()
{
  ...
  if(bnd.LinkType != linkUnbinded)
  {
    ...
    if(bnd.LinkType == linkPCF8575)
    {
     #if defined(USE_PCF8575_EXTENDER) && COUNT_OF_PCF8575_EXTENDERS > 0
        if(bnd.PowerPin != UNBINDED_PIN)
        {
          WORK_STATUS.PCF8575_PinMode(bnd.MCPAddress,bnd.PowerPin,OUTPUT);
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress,bnd.PowerPin,bnd.Level);      
        }
     #endif 
    }
...
void UniLoRaGate::sendWaterTankSettingsPacket()
{
   ...
   if(!loRaInited)
   {
	   ...
	   if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	   {
		    Serial4.println(F("LoRa: Send WATER TANK settings packet, LORA NOT INITED!!!"));
	   }
         #endif // WATER_TANK_MODULE_DEBUG    
      return;
   }
 ...
 //   #ifdef WATER_TANK_MODULE_DEBUG
	...
	//	 Serial4.println();
	//	 Serial4.println("**************************");
	//	 Serial4.println(F("LoRa: Send WATER TANK settings packet..."));
	//	 Serial4.print(F("maxWorkTime = "));
	//	 Serial4.println(waterTankSettingsPacket.maxWorkTime);
	//	 Serial4.print(F("distanceEmpty = "));
	//	 Serial4.println(waterTankSettingsPacket.distanceEmpty);
	//	 Serial4.print(F("distanceFull = "));
	//	 Serial4.println(waterTankSettingsPacket.distanceFull);
	//}
 //        #endif // WATER_TANK_MODULE_DEBUG
        ...
        LoRa.write((uint8_t*) &waterTankSettingsPacket,RADIO_PAYLOAD_SIZE);
			...
			if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
			{
				 ...
				 Serial4.println("**************************");
				 Serial4.println(F("LoRa: Send WATER TANK settings packet..."));
				 Serial4.print(F("packetType = "));
				 Serial4.println(waterTankSettingsPacket.packetType);
				 Serial4.print(F("maxWorkTime = "));
				 Serial4.println(waterTankSettingsPacket.maxWorkTime);
				 Serial4.print(F("distanceEmpty = "));
				 Serial4.println(waterTankSettingsPacket.distanceEmpty);
		...
		if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
		{
			 Serial4.println(F("LoRa: WATER TANK settings packet sent."));
			 Serial4.println();
		}
        #endif // WATER_TANK_MODULE_DEBUG       
...
void UniLoRaGate::sendFillTankCommand(bool on)
{
   ...
   if(!loRaInited)
   {
	   ...
	   if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	   {
		    Serial4.println(F("LoRa: Send WATER TANK COMMAND packet, LORA NOT INITED!!!"));
	   }
         #endif // WATER_TANK_MODULE_DEBUG    
      return;
   }
	...
	if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
	{
		 Serial4.print(F("LoRa: Send WATER TANK command packet with valve command: "));
		 Serial4.println(on);
	}
         #endif // WATER_TANK_MODULE_DEBUG
		...
		if (debug.WATER_TANK_MODULE_DEBUG_K == HIGH)
		{
			 Serial4.println(F("LoRa: WATER TANK command packet sent."));
		}
        #endif // WATER_TANK_MODULE_DEBUG       

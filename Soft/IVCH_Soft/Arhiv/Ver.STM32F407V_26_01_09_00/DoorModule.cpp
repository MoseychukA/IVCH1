...
void Door::update()
{
    ...
    if(!onMyWay)                                        // Тут контролируем параметры, но только тогда, когда дверь не движется
    {
            ...
            if (millis() - meteoTimer >= 2000ul)           // обновляем состояние метео не чаще, чем раз в 2 секунды
            {
                meteoTimer = millis();                     // сохраняем время последней проверки
 
                // здесь нужно привязать датчик дождя и ветра 
                // проверяем на ветер
                // ситуация такая, что если ветер часто меняет силу - не надо хлопать дверьми, поэтому нижний порог должен быть с гистерезисом.
                int16_t windSpeedNow1 = LogicManageModule->GetWindSpeed();  // Получить текущую скорость ветра
                GlobalSettings* settings = MainController->GetSettings();
                hurricaneSpeed = settings->GetHurricaneSpeed();             // Получить уставку (порог) ураганного ветра

                const int16_t windSpeedHisteresis = 3 * 100;                // 3 метра в секунду - гистерезис

                // проверяем на ураган
                int16_t windSpeedHighBorder = (hurricaneSpeed * 100);
                int16_t windSpeedLowBorder = windSpeedHighBorder - windSpeedHisteresis;
                if (windSpeedLowBorder < 0)
                    windSpeedLowBorder = 0;

                bool shouldOpen_tmp = shouldOpen;   // Запомним текущее значение управления дверьми


                //здесь закрываем двери при сильном ветре.
                if (windSpeedNow1 >= windSpeedHighBorder)  // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
                {
                     Serial4.println("windSpeedHighBorder");
                    DoorSettings MeteoSettings = settings->GetDoorSettings(0);
                    bool wingMeteoFlag = MeteoSettings.wing;         // Проверить, есть разрешение на закрытие дверей
                    if (wingMeteoFlag)
                    {
                        shouldOpen = false;
                        if (openFlag)
                        {
                             ...
                             Serial4.print("DOOR #");
                             Serial4.print(channel);
                             Serial4.println(" SHOULD BE CLOSED!");
                            #endif
                            close();
                        }
                    }
                ...
                if (windSpeedNow1 <= windSpeedLowBorder)
                {
                    shouldOpen = shouldOpen_tmp;    //Ураган закончился. Восстановим текущее значение управления дверьми
                }
                bool hasRain = LogicManageModule->HasRain();

                if (hasRain)
                {
                     Serial4.println("hasRain");
                    DoorSettings MeteoSettings = settings->GetDoorSettings(0);
                    bool wingMeteoFlag = MeteoSettings.rain;         // Проверить, есть разрешение на закрытие  дверей
                    if (wingMeteoFlag)
                    {
                        shouldOpen = false;
                    }
                ...
                {
                    shouldOpen = shouldOpen_tmp;    //Дождь закончился. Восстановим текущее значение управления дверьми
                }
        ...
        if(millis() - controlTimer >= 5000ul)          // обновляем состояние не чаще, чем раз в 10 секунд
        {
            controlTimer = millis();                   // сохраняем время последней проверки
            #ifdef DOOR_DEBUG      
                 Serial4.print("DOOR #");
                 Serial4.print(channel);
                 Serial4.println(" WANT TO CHECK PARAMETERS...");
            #endif          
        ...
        // получаем текущее время
            ...
            // если датчик не привязан или с него нет данных - открываем дверь.
                     ...
                     Serial4.print("DOOR #");
                     Serial4.print(channel);
                     Serial4.println(" IN WORKING TIME!");
            #endif        
                
                if(canWorkWithSensor)                       // можно работать с дверью на открытие. Надо проверить датчик температуры
                {
                shouldOpen = true;                          // флаг, что мы можем открывать двери. они открываются только по времени, если датчик температуры не привязан, или - с него нет данных.
                ...
                int8_t sensorIndex = settings->sensorIndex;
                    ...
                    {
                        if(sensorIndex >= tempSensorsCount)  // если индексы датчиков DS1820 закончились, смотрим датчики влажности воздуха
                        {
                        ...
                        if(module) // есть модуль
                        {
                            ...
                            if(sensorState->HasData()) // есть данные на датчике
                            {
                                ...
                                if(t.Value >= settings->minOpenTemp)     // если текущая температура больше либо равна уставке - можем открывать
                                {
                                         ...
                                         Serial4.print("DOOR #");
                                         Serial4.print(channel);
                                         Serial4.print(", GOOD TEMP: ");              
                                         Serial4.print(t.Value);
                                         Serial4.print(", BORDER: ");
                                         Serial4.println(settings->minOpenTemp);
                                    #endif
                                    shouldOpen = true;
                                }
                                else // проверяем на нижний порог, он должен быть меньше на 5 градусов, чем уставка - и тогда мы закроем дверь
                                {
                                    if(t.Value <= (settings->minOpenTemp - 5)) // текущая температура на 5 градусов ниже уставки
                                    {
                                             ...
                                             Serial4.print("DOOR #");
                                             Serial4.print(channel);
                                             Serial4.print(", TOO SMALL TEMP: ");              
                                             Serial4.println(t.Value);
                                        #endif                            
                    ...
                    if (windSpeedNow1 >= windSpeedHighBorder)  // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
                    {
                        // Serial4.println("windSpeedHighBorder");
                        DoorSettings MeteoSettings = settings->GetDoorSettings(0);
                        bool windMeteoFlag = MeteoSettings.wing;         // Проверить, есть разрешение на закрытие дверей при урагане
                        if (windMeteoFlag)
                        {
                            shouldOpen = false;
                        }
            ...
            else
            {
                ...
                if(openFlag) // если дверь открыта, то закрываем
                {
                         ...
                         Serial4.print("DOOR #");
                         Serial4.print(channel);
                         Serial4.println(" SHOULD BE CLOSED BECAUSE OUT OF DIA"); 
            #endif
                    close();
                }
            }
...
{
#ifdef DOOR_DEBUG
   Serial4.print("SET OUTPUTS FOR DOOR #");
   Serial4.print(channel);
   Serial4.print(": ");  
   Serial4.print(state1);
   Serial4.print(",");
   Serial4.println(state2);
#endif
 ...
 DoorBinding bnd = HardwareBinding->GetDoorBinding(channel);
  ...
  else if(bnd.LinkType == linkPortExpander)
  {
    ...
    if(bnd.Pin2 != UNBINDED_PIN)
    {
      ...
      if (bnd.Pin1 != UNBINDED_PIN)
      {
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin1, state1);
      }
      if (bnd.Pin2 != UNBINDED_PIN)
      {
          WORK_STATUS.PCF8575_PinWrite(bnd.MCPAddress, bnd.Pin2, state2);
      }
    #endif
  }
}

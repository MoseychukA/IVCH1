...
void setup() 
{
   ...
   /* rtc.setTime(0, 16, 18, 7, 18, 1, 2026);*/
...
//-----------------------------------------------------------------------
    ...
    sim.setPeriodMs(600000); // 1 минута
...
void loop() 
{
    ...
    //        Serial.print(second);
    //        Serial.print(" tzq="); Serial.println(tzq);
    ...
    if (sim.hasNewTime())
    {
        ...
        if (SIM800TimeAsync::parseCCLK(raw, year, month, day, hour, minute, second, tzq))
        {
            ...
            // День недели по ЛОКАЛЬНОЙ дате
            uint8_t dow0 = dow0_sun(year, month, day); // 0..6 (Sun..Sat)
            uint8_t dsDow = ds3231DowFromDow0(dow0); // 1..7 (Mon..Sun)

            // Запись ЛОКАЛЬНОГО времени в DS3231
            rtc.setTime((uint8_t)second, (uint8_t)minute, (uint8_t)hour,
                dsDow, (uint8_t)day, (uint8_t)month, (uint16_t)year);

            RTCTime nowLocal = rtc.getTime();
            Serial.print(F("RTC set LOCAL:"));
            Serial.print(rtc.getDateStr(nowLocal));
            Serial.print(F(" "));
            Serial.println(rtc.getTimeStr(nowLocal));
        }
        else
        {
            Serial.println(F("CCLK parse failed -> RTC not updated"));

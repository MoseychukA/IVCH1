...
// -------------------- Channel RX stamps (proxy for "time data is coming") --------------------
static uint32_t gChIp1RxMs = 0;
static uint32_t gChIp2RxMs = 0;
static uint32_t gChGsmRxMs = 0;

// GSM change detection
static int8_t gPrevCsq = -127;
static uint8_t gPrevBars = 255;
static bool gPrevReg = false;

static void updateChannelRxStamps(uint32_t now)
{
	// IP1/IP2:treat non-zero IP as "channel alive"
	if (!isZeroIpStr(gIp1Str)) gChIp1RxMs = now;
	if (!isZeroIpStr(gIp2Str)) gChIp2RxMs = now;

	// GSM:detect any change as "data arrived"
	if (gPrevCsq == -127) {
		gPrevCsq = gCsqRssi;
...
static inline bool chIp1Ok(uint32_t now)
{
	return (syncData.netEnable != 0) &&
		isFreshByPeriod(gChIp1RxMs, periodIdxToMs(syncData.netPeriodIdx), now);
}
static inline bool chIp2Ok(uint32_t now)
{
	return (syncData.net2Enable != 0) &&
		isFreshByPeriod(gChIp2RxMs, periodIdxToMs(syncData.net2PeriodIdx), now);
}
static inline bool chGsmOk(uint32_t now)
{
	return (syncData.gsmEnable != 0) &&
		isFreshByPeriod(gChGsmRxMs, periodIdxToMs(syncData.gsmPeriodIdx), now);
}
...
static inline void setLedPinIfDefined(int pin, bool on)
{
	if (pin < 0) return;
	pinMode(pin, OUTPUT);
...
static inline void updateLedsByChannels(uint32_t now)
{
#ifdef LED1_PIN
	setLedPinIfDefined(LED1_PIN, chGpsOk(now));
#endif
#ifdef LED2_PIN
	setLedPinIfDefined(LED2_PIN, chIp1Ok(now));
#endif
#ifdef LED3_PIN
	setLedPinIfDefined(LED3_PIN, chIp2Ok(now));
#endif
#ifdef LED4_PIN
	setLedPinIfDefined(LED4_PIN, chGsmOk(now));
#endif
}

// -------------------- Top marker [O]/[X] (SW5 only,anti-flicker) --------------------
// Cache last SW5 state; redraw marker only when state changes.
// Use Force() on first draw of each page.
...
static inline void drawSw5MarkerTopForce(TFT_eSPI* d, int y)
{
	...
	// clear ONLY where marker is (avoid damaging long headers)
...
// Helper:IP to octets (needed for Page2/Page3)
...
// -------------------- GPS helpers --------------------
// HOME:HDOP removed; FIX/NOFIX colored by renderer
...
public:
	...
	{
		// update marker only if SW5 changed
...
public:
	...
	void update() override
	{
		...
		if (strcmp(_statLine, line) != 0) {
			...
			drawStatusLine(); // will redraw marker (force) after status
		}
	}
...
private:
	...
	void drawStatusLine()
	{
		...
		// STATUS draw can wipe marker area -> redraw marker FORCE (no flicker unless STATUS changes)
		drawSw5MarkerTopForce(d, Y_TITLE);
	}
...
private:
	...
	void drawStatusLine()
	{
		...
		// redraw marker FORCE after status update
		drawSw5MarkerTopForce(d, Y_TITLE);
	}
...
void Menu::setup(const String& ver)
{
	...
	// init SW5 cache on boot (prevents unnecessary redraws)
...
void Menu::update()
{
	...
	// Update stamps/LEDs always
	updateChannelRxStamps(now);
	updateLedsByChannels(now);
...
void Menu::drawHomeDynamic()
{
	...
	// Update SW5 marker only if changed
	...
	// LINE1 GPS:"GPS:" white,FIX green / NOFIX red,sats,right [T]/[X]
	{
	...
	// LINE4 GSM:"GSM:" white,status colored,right [T]/[X]
	{
...
void Menu::drawPageSelect()
{
	...
	// Force marker on this page draw (prevents missing marker)
...
void Menu::drawTestPage()
{
	...
	// Force marker on this page draw
	drawSw5MarkerTopForce(tft, 10);
}

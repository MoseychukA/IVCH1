...
private:
	...
	void buildLine(uint8_t i, char* out, size_t n) const {
		if (_mode == MODE_GPS) {
			...
			else snprintf(out, n, "2) ПЕРИОД: %s", kPeriodsName[_tmp.gpsPeriodIdx % 6]);
			return;
		}
		...
		if (_mode == MODE_NET) {
			...
			else snprintf(out, n, "3) ПЕРИОД: %s", kPeriodsName[_tmp.netPeriodIdx % 6]);
			return;
		}

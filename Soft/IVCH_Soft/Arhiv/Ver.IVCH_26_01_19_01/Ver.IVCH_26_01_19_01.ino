...
void setup() 
{
   ...
   // sim.setDebug(&Serial, false);  // Не выводить дебаг

    sim.begin(9600);

    sim.setHealthPingMs(60000);
    sim.setStatusPollMs(5000); // например 5 секунд
...
void loop() 
{
    ...
    if (gNetUpdated) 
    {
        ...
        // Рисуем статус только когда он изменился
      //  menu_start.drawModemStatus();
    }
...
void SD_setup()
{
    Serial.print("\nInitializing SD card...");
     ...
     if (gNetUpdated) 
     {
         ...
         char st[48];
         snprintf(st, sizeof(st), "NET:%d CREG:%u RSSI:%ddBm [%u]",
             gNetRegistered ? 1 : 0,
             (unsigned)gCregStat,
             (int)gRssiDbm,
             (unsigned)gSignalBars);

        // tft.drawString(st, 5, 5, 2);
     }
}

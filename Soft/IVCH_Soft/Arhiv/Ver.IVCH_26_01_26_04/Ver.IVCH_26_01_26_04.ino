...
// -------------------- настройки --------------------
static constexpr uint16_t LOG_KEEP_DAYS = 30; // сколько суток хранить
...
// 8.3 file name:"IVyymmdd.LOG" (8 chars + ".LOG") => 12 bytes + '\0' = 13
...
//// days-from-civil (UTC-like) for age compare (only date matters)
//static int32_t daysFromCivil(int y, int m, int d)
//{
//	y -= (m <= 2);
//	const int era = (y >= 0 ? y : y - 399) / 400;
...
static bool parseDailyLogName(const char* nm, int& y, int& m, int& d)
{
	...
	// Expect exactly 12 chars:I V y y m m d d . L O G
	if (strlen(nm) != 12) return false;
...
static char gLogCurName[13] = { 0 }; // current opened log name (8.3)
static uint32_t gCleanupKeyYmd = 0; // yyyymmdd of last cleanup run
...
static void SdLogCleanupOldInternal(const RTCTime& nowT, const char* keepName)
{
	...
	const int yNow = (int)nowT.year;
	const int mNow = (int)nowT.month;
	const int dNow = (int)nowT.dayOfMonth;
	const int32_t nowDays = daysFromCivil(yNow, mNow, dNow);
	...
	while (true) {
		...
		// only our daily logs + NO_RTC
		if (strcmp(nm, kLogNoRtcName) == 0) {
			// If RTC is valid,we can remove "no rtc" log to avoid garbage accumulation.
			// But never delete currently opened file.
			f.close();
		...
		int y, m, d;
		if (!parseDailyLogName(nm, y, m, d)) { f.close(); continue; }

		f.close();

		// don't touch current file
		if (keepName && strcmp(keepName, nm) == 0) continue;

		const int32_t fileDays = daysFromCivil(y, m, d);
		const int32_t ageDays = nowDays - fileDays;

		// delete strictly older than KEEP_DAYS
...
bool SdLogOpenIfNeeded()
{
	...
	// auto-clean old logs once per day (only when RTC valid)
	if (rtcDateValid(t)) {
		uint32_t key = (uint32_t)t.year * 10000UL + (uint32_t)t.month * 100UL + (uint32_t)t.dayOfMonth;
		if (gCleanupKeyYmd != key) {
			gCleanupKeyYmd = key;
			// Close current file before deleting others? Not necessary; we never delete keepName.
...
void SdLogClose()
{
	...
	gLogCurName[0] = 0;
	// cleanup key оставляем,чтобы не запускать cleanup повторно сразу
}
...
void SdLogLine(const char* msg)
{
	...
	// open/rotate (and cleanup once per day when RTC valid)

...
public:
	...
	void begin(); // поднимет server + (опционально) создаст дефолтную auth запись в EEPROM
	void tick();
...
private:
	...
	static bool startsWithI(const char* s, const char* pref); // case-insensitive prefix
	...
	// NOTE:используем свободный диапазон 0x0190..0x01BF
	struct AuthRec {
		char user[32];
		char pass[32];
	};

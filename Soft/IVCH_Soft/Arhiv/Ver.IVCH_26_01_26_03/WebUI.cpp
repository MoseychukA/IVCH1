...
// Confirmation page (cannot call member sendHeader without object,so we write header directly)
static void pageLogClearConfirm(EthernetClient& c)
{
	...
	htmlHeader(c, F("ОЧИСТКА LOG"));

	c.print(F("<div class='box'>"
		"<h3>Очистить лог-файл?</h3>"
		"<p><code>ivch.log</code></p>"
		"<p>"
		"<a href='/log/clear?confirm=1'><button>ДА,ОЧИСТИТЬ</button></a> "
...
void WebUI::handleClient(EthernetClient& c)
{
	...
	{
		File f = SD.open("ivch.log", FILE_READ);
		if (!f) {
			sendHeader(c, 200, F("text/plain; charset=utf-8"));
			c.print(F("LOG not available (SD not mounted or ivch.log not found)\n"));
			c.stop();
			return;
		}
		...
		c.print(F("Cache-Control:no-store\r\n"));
		c.print(F("Content-Type:text/plain; charset=utf-8\r\n"));
		c.print(F("Content-Disposition:attachment; filename=\"ivch.log\"\r\n"));
		c.print(F("\r\n"));
	...
	else if (isGet && strcmp(path, "/log/clear") == 0)
	{
		...
		if (SD.exists("ivch.log")) ok = SD.remove("ivch.log");
		else ok = true;

		// Recreate empty file
		File nf = SD.open("ivch.log", FILE_WRITE);
		if (nf) nf.close();
		...
		if (ok) {
			...
			sendHeader(c, 200, F("text/plain; charset=utf-8"));
			c.print(F("LOG clear FAILED (file may be open). Close logger or reboot and retry.\n"));
		}

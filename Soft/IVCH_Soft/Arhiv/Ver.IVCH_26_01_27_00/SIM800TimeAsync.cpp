...
// SD logger (timestamp is added by SdLogLine itself)
extern void SdLogLine(const char* s);

// STRICT:only FAIL for transport/service level here (timeout/no_network).
// OK and FAIL(parse/bad_cclk) MUST be logged where rtc.setTime() is applied (.ino).
static inline void gsmFailLog(const char* reason)
{
	char line[96];
	snprintf(line, sizeof(line), "TIME GSM FAIL %s", reason ? reason : "fail");
	SdLogLine(line);
}

static constexpr int GSM_MIN_YEAR_VALID = 2020;
...
void SIM800TimeAsync::requestTimeNow() {
	...
	// mark external request
	_extReqActive = true;
	_extReqResultReady = false;
	_extReqStartMs = millis();
}
...
bool SIM800TimeAsync::consumeExternalTimeResultFlag()
{
	// true только если текущий hasNewTime() относится к внешнему запросу
	bool v = (_extReqActive && _extReqResultReady);
	// сбрасываем только флаг "результат готов" (а активность тоже завершаем здесь)
	if (v) {
		_extReqActive = false;
		_extReqResultReady = false;
	}
...
void SIM800TimeAsync::_serviceState() {
	...
	// External request watchdog:120s -> FAIL timeout
	if (_extReqActive && (now - _extReqStartMs) > 120000UL) {
		_extReqActive = false;
		_extReqResultReady = false;
		gsmFailLog("timeout");
	}
	...
	case State::AT_PING:
		if (!_atPending) _startAT("AT", "OK", 1500);
			...
			if (_atOk) { _recoveryUsed = 0; _enter(State::IDLE); }
			else if (_atDoneTimeout()) {
				if (_recoveryUsed < _maxRecoveryAttempts) { _recoveryUsed++; _enter(State::PWRKEY_PULSE); }
				else _enter(State::STOPPED);
			}
		}
		return;
	...
	case State::STAT_CREG:
		if (!_atPending) _startAT("AT+CREG?", "+CREG:", 1500);
			...
			else if (_atDoneTimeout()) _enter(State::STAT_CSQ);
		}
		return;

	case State::STAT_CSQ:
		if (!_atPending) _startAT("AT+CSQ", "+CSQ:", 1500);
		else {
			if (_atMatchedExpect()) {
			...
			else if (_atDoneTimeout()) _enter(State::IDLE);
		}
		return;
	...
	case State::PROBE_AT:
		if (!_atPending) _startAT("AT", "OK", 1500);
			...
			if (_atOk) { _recoveryUsed = 0; _enter(State::INIT_ATE0); }
			else if (_atDoneTimeout()) {
				if (_recoveryUsed < _maxRecoveryAttempts) { _recoveryUsed++; _enter(State::PWRKEY_PULSE); }
				else _enter(State::STOPPED);
			}
	...
	case State::WAIT_URC:
		if (_seenRDY || _seenCallReady || _seenSMSReady || (now - _stEnterMs) > 8000) _enter(State::WAIT_AT_READY);
		return;
	...
	case State::INIT_SAVE:
		...
		else if (_atOk || _atDoneTimeout()) { _recoveryUsed = 0; _enter(State::IDLE); }
		return;
	...
	case State::WAIT_NETWORK:
		if (!_atPending) _startAT("AT+CREG?", "+CREG:", 1500);
			...
			if (_atDoneTimeout()) {
				if (now - _stEnterMs > 60000) {
					// external request -> FAIL no_network
					if (_extReqActive) {
						_extReqActive = false;
						_extReqResultReady = false;
						gsmFailLog("no_network");
					}
					_enter(State::IDLE);
				}
				else {
	...
	case State::REQ_CCLK:
		if (!_atPending) {
			...
			if (_atMatchedExpect()) {
				...
				if (!_isCCLKValid(_lastCCLK, GSM_MIN_YEAR_VALID) && _ntp.enableFallback && _ntp.apn && _ntp.apn[0]) {
					_enter(State::NTP_CGATT);
				}
				else {
					// We got a CCLK response for external request -> let .ino log OK/FAIL(parse/bad_cclk)
					if (_extReqActive) _extReqResultReady = true;
					_enter(State::IDLE);
				}
			}
			else if (_atDoneTimeout()) {
	...
	case State::NTP_CNTP_SET: {
		...
		else if (_atOk || _atDoneTimeout()) _enter(State::NTP_CNTP_START);
		return;
	}

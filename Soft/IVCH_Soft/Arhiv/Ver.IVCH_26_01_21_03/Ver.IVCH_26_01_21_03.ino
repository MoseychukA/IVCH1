...
static void updateIpFeed1Hz()
{
	...
	// Internet1:текущий W5500
	IPAddress ip = Ethernet.localIP();
	char buf[16];
	snprintf(buf, sizeof(buf), "%u.%u.%u.%u", ip[0], ip[1], ip[2], ip[3]);

	bool changed = (strcmp(gIp1Str, buf) != 0);
	if (changed) {
		strncpy(gIp1Str, buf, sizeof(gIp1Str) - 1);
		gIp1Str[sizeof(gIp1Str) - 1] = 0;
	}

	// Internet2 пока не реализован
	if (strcmp(gIp2Str, "0.0.0.0") != 0) {
		strncpy(gIp2Str, "0.0.0.0", sizeof(gIp2Str) - 1);
		gIp2Str[sizeof(gIp2Str) - 1] = 0;
		changed = true;
	}

	if (changed) gIpUpdated = true;
}

...
public:
	...
	bool hasNewTime(); // true один раз после обновления (и сброс флага)
	String lastCCLKRaw() const;// строка +CCLK:...
	bool lastTimeValid(int minYear = 2020) const;

	// NEW:признак,что полученное время относится к внешнему запросу (planner/UI).
	// Использование в .ino:
	// if (sim.hasNewTime()) { bool ext = sim.consumeExternalTimeResultFlag(); ... log OK/FAIL only if ext ... }
	bool consumeExternalTimeResultFlag();
...
private:
	...
	// NEW:external request tracking
	bool _extReqActive = false;
	bool _extReqResultReady = false; // set when we obtained CCLK response for external request
	uint32_t _extReqStartMs = 0;

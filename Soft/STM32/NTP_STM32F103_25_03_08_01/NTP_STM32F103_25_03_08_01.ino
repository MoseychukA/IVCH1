...
NTPClient timeClient(Udp, timeServer, (3600 * TIME_ZONE_OFFSET_HRS), NTP_UPDATE_INTERVAL_MS);
...
int yearn;
byte monthn, dayn, hourn, minuten, secondn, hundredths;
unsigned long date, times, age;
...
void setup() 
{
  ...
  SerialUSB.println(F("We are waiting for the time GPS!"));
...
void testSAT()
{
    Serial.print(F("testSAT!"));     //  Выводим информацию 
    gps.read();
    newTimeGPS = true;
    ...
    {
        newTimeGPS = false;
        Serial.println(F(" The time is unreliable."));     //  Выводим информацию о недостоверном времени.
        //viewSattelite();
    }                                             //
    if (gps.errDat)
    {
        newTimeGPS = false;
        Serial.println(F(" The date is unreliable."));      //  Выводим информацию о недостоверной дате.
    }                                             //
//   Завершаем строку:                             //
    Serial.print("\r\n");
...
void commandChangeIp(uint16_t ipAddress[5])
{
    ...
    {
        Serial.println(F("Wrong IP in change ip command \n"));
        return;
    }
...
void processGNRMC(HardwareSerial* serial_pointer)
{
    ...
    if (serial_pointer->available() > 0)
    {
        ...
        if (renew && lat[0] == 'A' && (rtc.getEpoch() - Time_last_sync) > 10)
        {
            ...
            /* установить внутреннее время с учетом времени, полученного от GPS */
...
void EthernetTime()
{
    ...
    if (timeClient.updated())
        Serial.println(F("******** EthernetTime UPDATED********"));
    else
        Serial.println(F("****** EthernetTime NOT UPDATED******"));
...
void processNTP()
{
    ...
    if (packetSize)
    {
        ...
        getRtcDate();
        timestamp = numberOfSecondsSince1900Epoch(yearn, monthn, dayn, hourn, minuten, secondn);

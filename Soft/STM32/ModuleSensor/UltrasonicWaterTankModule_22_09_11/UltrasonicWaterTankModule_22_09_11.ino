...
int num_message = 0;                         // номер текущего сообщения  
char message1[] = "Distance to object..   ";
char message2[] = "Change distance!!      ";
char message3[] = "Дист.вне диапазона!!   ";
//char message3[] = "Distance out of range =";
char message4[] = "!! Emergency level!!   ";
...
{  
  ...
  if(loRaInited)
  {
	...
	display.println(F("LoRa inited. Ok! "));
	display.display();
...
void updateSensor(bool immediate)
{
  ...
  switch (num_message)
  {
	  case 1:
	  {
		  display.print(utf8rus("Расстояние"));   //Distance out of range = 
		  //display.print(utf8rus(" до поверхн"));   //Distance out of range = 

		 // display.print(message1);   //Distance to object..  
	  }
	  break; // 
	  case 2:
	  {
		  display.print(utf8rus("Изменить "));   //Distance out of range = 
		  display.print(utf8rus("дистанцию"));   //Distance out of range = 
		  //display.print(message2);   //Change distance!! 
	  }
	  break; // 
	  case 3:
	  {
		  display.print(utf8rus("Дист."));   //Distance out of range = 
		  display.print(utf8rus("вне диапазона"));   //Distance out of range = 
		 // display.print(utf8rus(message3));   //Distance out of range = 
	  }
	  break; // 
	  case 4:
	  {
		  display.print(utf8rus("Аварийный "));   //Distance out of range = 
		  display.print(utf8rus("уровень"));   //Distance out of range = 
  ...
  {
	  display.print(messagedDistance);
  }

  display.print("cm.  ");
  display.setTextSize(1);            // Normal 1:1 pixel scale
  display.setCursor(75, 16);         // Start at top-left corner
  display.print("High=");
  display.println(distanceFull);
  display.setCursor(75, 25);         // Start at top-left corner
  display.print("Low=");
  display.println(distanceEmpty);
  display.display();
...
void processSettingsPacket(uint16_t _distanceEmpty, uint16_t _distanceFull, uint32_t _maxWorkTime)
{
  ...
  display.setTextSize(1);      // Normal 1:1 pixel scale
  display.setCursor(0, 0);     // Start at top-left corner
  //display.print(F("*SettingsPacket*"));
  //display.setTextSize(1);           // Normal 1:1 pixel scale
  //display.setCursor(0, 8);          // Start at top-left corner
  display.print("controllerID = ");
  display.println(controllerID);
  display.print("Time = ");
  display.println(maxWorkTime/1000);
  display.print("dist.Empty = ");
  display.println(distanceEmpty);
  display.print("dist.Full  = ");
  display.println(distanceFull);
  display.display();
  //delay(1000);
  display.clearDisplay();
...
void setup()
{
  bool serialStarted = false;
	...
	//display.println(F("S T A R T!"));
	display.println(utf8rus("ВКЛЮЧЕНИЕ"));
	display.display();
	delay(1000);

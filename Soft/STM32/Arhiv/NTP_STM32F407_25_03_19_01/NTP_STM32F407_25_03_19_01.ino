...
HardwareSerial SerialRS2322(PC7, PC8);     //USART6
...
HardwareSerial SerialSIM800C(PA1, PA0);    //USART4
...
/* Test diode*/
const int ledPin = PE14;// RED LED the number of the LED pin
const int ledPin2 = PE15;// RED LED the number of the LED pin
int ledState = LOW;             // ledState used to set the LED
int ledState2 = HIGH;             // ledState used to set the LED
const int w5500rst = PB1;// RED LED the number of the LED pin
const int w5500int = PB0;// RED LED the number of the LED pin
...
void setup() 
{
 ...
 // while (!Serial && millis() < 5000);
  ...
  digitalWrite(ledPin2, HIGH); // установите светодиод 
 
  pinMode(w5500rst, OUTPUT);
  digitalWrite(w5500rst, HIGH); // 

  pinMode(TFT_BL, OUTPUT);
  digitalWrite(TFT_BL, LOW); // установите светодиод 
  ...
  digitalWrite(TFT_BL, HIGH); // установите светодиод
...
void loop()
{
   processNTP(); // обрабатываем приходящие NTP запросы. Спрашиваем время у нас
   ...
   if (ledState2 == LOW)
   {
       if (currentMillis - previousMillisLED2 >= intervalLED)
       {
           ...
           ledState2 = HIGH;
           digitalWrite(ledPin2, ledState2); // установите светодиод с помощью ledState переменной:
       }
   }
}

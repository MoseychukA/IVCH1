...
#endif
USBSerial SerialUSBU;

RTClock rtclock (RTCSEL_LSE); // initialise
int timezone = 3;      // change to your timezone
...
void ntp_recv_and_respond(uint16_t dest_port, uint8_t src_ip[4], uint16_t src_port, const char *data, uint16_t len)
{
   ...
   SerialUSBU.println("Packet length: "+ String(len));
   ...
   SerialUSBU.print("Timestamp: ");
   SerialUSBU.println(timeStamp);
   SerialUSBU.print("UNIX time: ");
   SerialUSBU.println(timeStamp-STARTOFTIME);
   SerialUSBU.print("src_ip: ");
   SerialUSBU.print(src_ip[0]);
   SerialUSBU.print('.');
   SerialUSBU.print(src_ip[1]);
   SerialUSBU.print('.');
   SerialUSBU.print(src_ip[2]);
   SerialUSBU.print('.');
   SerialUSBU.println(src_ip[3]);
   SerialUSBU.print("dest_port: ");
   SerialUSBU.println(dest_port);
   SerialUSBU.print("src_port: ");
   SerialUSBU.println(src_port);
   ...
   SerialUSBU.print("NOW IS: ");
   SerialUSBU.print(rtclock.getTime());
   SerialUSBU.println();
   ...
   if(serial_pointer->available() > 0)
   {
      ...
      switch (state )
      {
         ...
         if(',' == b)
         {
            ...
            //SerialUSBU.print("time_GPS = " + time_GPS + " lat = " + lat + " lat_s =" + lat_s + " lon = " + lon + " lon_s = " + lon_s  + " qual = " +qual + " sats = " + sats + " date_GPS = " + date_GPS + "\r\n");
            renew = true;
            newTimeGPS = true;
            state = '0';
         }
...
/* if found flag A, we sync every 300 seconds todo : setup a stack with last 5 values from gps */
      if (renew&&lat[0]=='A'&&(rtclock.getTime()-Time_last_sync )> 3)
      {
         ...
         SerialUSBU.print("time_GPS = " + time_GPS + " lat = " + lat + " lat_s =" +
      ...
      /* установить маркер V в 0 регистре modbus, - время НЕдействительно */
